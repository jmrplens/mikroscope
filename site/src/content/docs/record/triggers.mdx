...
2026-09-17) was <Measured id="ring.lineBytes" />; lines at the default floors were not
...
`rate × buffer ×` <Measured id="ring.lineChargedBytes" /> plus `CAPTURE_MB` exceeds it, the agent
...
budget can still draw the warning. [The cost of the observer](/mikroscope/cost/) explains why that
...
The relay transport pulls through `/tool fetch`, which returns at most <Measured id="relay.replyMaxBytes" /> and sends no token. A capture at the defaults is about 330 kB, five times the fetch limit. Fetch captures from a host that
...
toggle bar, and it works on three of them: on InfluxDB and PostgreSQL it reads the
`mikroscope_trigger` rows, on Prometheus the collector's
`mikroscope_trigger_fired_total`. The Elasticsearch and Graphite dashboards carry
that same PromQL expression against their own datasource, so their `triggers`
annotation, like their `detections` one, returns nothing. The capture itself stays
...
job on the collector carries them. [Prometheus](/mikroscope/sinks/prometheus/)
has the detail.
