...
description: Which kernel sources the agent reads every tick and which less often, the named reason each reports, and FLOOR_HZ, which turns every floor off.
head:
  - tag: title
    content: "How often each RouterOS kernel source is read | mikroscope"
---
...
- **The kernel log ([`/dev/kmsg`](https://docs.kernel.org/admin-guide/abi-testing.html#symbols-under-dev-kmsg))** is drained every tick and never slowed. An event's value
...
**Temperature.** On the reference RB5009 both zones declare a [`polling_delay`](https://docs.kernel.org/driver-api/thermal/sysfs-api.html) of 1 000 ms
...
The reference RB5009's [cpufreq governor](https://docs.kernel.org/admin-guide/pm/cpufreq.html#policy-interface-in-sysfs) read `userspace` on 2026-09-14, so by the rule above its
...
The floors are named constants in <Src path="internal/agent/source.go" /> rather than numbers buried
in the logic.
