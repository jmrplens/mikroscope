...
description: Which files in a RouterOS container are the router’s and which its own, why privileged does not change that, and the conntrack count that gets through.
head:
  - tag: title
    content: "What a RouterOS container can see of the kernel | mikroscope"
---
...
- [`/proc/stat`](https://man7.org/linux/man-pages/man5/proc_stat.5.html), per core
- `/proc/interrupts` and `/proc/softirqs`
- [`/proc/net/softnet_stat`](https://docs.kernel.org/filesystems/proc.html#networking-info-in-proc-net): drops and time squeezes in the kernel's receive path. It lives
...
[network namespace](https://man7.org/linux/man-pages/man7/network_namespaces.7.html). Inside the container they describe the container's own veth: 4 packets
...
[`/proc/slabinfo`](https://man7.org/linux/man-pages/man5/slabinfo.5.html), and the slab allocator is global: the `nf_conntrack` cache's active-object
count is the router's real conntrack population. On the RB5009 on 2026-09-12 it
read <Measured id="conntrack.slabDiscovery" /> in the discovery container and <Measured id="conntrack.slab" /> from the agent the same day, while the
