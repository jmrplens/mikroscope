...
description: Why a MikroTik router’s CPU reading is no finer than the kernel’s 10 ms tick, the one source that reads beneath it, and how far back the agent remembers.
head:
  - tag: title
    content: "Why RouterOS CPU readings stop at the kernel tick | mikroscope"
---
...
A tick lasts <Measured id="tick.ms" />. [`/proc/stat`](https://man7.org/linux/man-pages/man5/proc_stat.5.html) does not count time, it counts
...
[`perf_event_open`](https://man7.org/linux/man-pages/man2/perf_event_open.2.html), which the agent collects as the `perf` source when the container is
...
Do not expect a finer clock from [PSI](https://docs.kernel.org/accounting/psi.html), whose stall totals are in microseconds, or from
[`schedstat`](https://docs.kernel.org/scheduler/sched-stats.html), whose run and wait times are in nanoseconds. The RB5009's RouterOS 7.24.2 kernel
...
carries a `full` line, all zeros when read again on 2026-09-24: the kernel's
PSI documentation says CPU `full` is undefined system-wide and has been reported
since 5.13 as zero, so an older kernel has no such line and the parser accepts
either.
