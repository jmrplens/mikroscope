...
like. It is read from <Src path="internal/agent/http.go" />, <Src path="internal/agent/capture.go" />, <Src path="internal/agent/source.go" />
and <Src path="internal/sample/sample.go" />.
...
response (`ReadHeaderTimeout` and `WriteTimeout` in <Src path="internal/agent/agent.go" />).
...
and sends no `Authorization` header (<Src path="internal/transport/transport.go" />), so
...
2026-09-21: three connections lived <Measured id="stream.lifetime" /> and
carried <Measured id="stream.lines" /> lines each. **A consumer of `/stream`
