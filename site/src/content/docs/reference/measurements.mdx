...
description: Every InfluxDB measurement and SQL table mikroscope writes, with tags, fields, columns, keys, units, and whether each value is a delta or a level.
---
...
read from <Src path="internal/sinks/influx.go" />, <Src path="internal/sinks/device.go" />
and <Src path="internal/sinks/sql.go" />. The two stores do not hold the same set;
[where they differ](#where-the-two-stores-differ) is the last section.
...
- `forward --stdout lp` and `forward --telegraf` render with the same encoder,
  ...
  file <Src path="internal/sinks/telegraf.go" /> records 12 renders of an 8-name map giving 7
...
(<Src path="internal/dashboards/panels_p5.go" />); no source in the repository says why.
...
The last four are the agent's [`/sampler`](/mikroscope/reference/http/#get-sampler), read when
...
| `mikroscope_perf`    | `time, host, counter, cpu`            | `host`, `counter`, `cpu`, `count` (delta), `enabled_ns`, `running_ns` (`NULL` where the kernel reported none; <PrivilegedOnly />) |
