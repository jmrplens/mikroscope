...
page lists both, from the Go files in <Src path="cmd/mikroscope/" /> and
from <Src path="internal/agent/config.go" />.
...
- Quote a URL that holds `&` when it lives in a file you `source`:
  unquoted, `&` is a shell operator. <Src path=".env.example" /> quotes
...
| Variable                  | Flag             | Read by                                               | Default in the code          | Value in <Src path=".env.example" /> |
| ------------------------- | ---------------- | ----------------------------------------------------- | ---------------------------- | ----------------------- |
...
| ------------------------- | -------------- | --------------------------- | -------------------------------------------------------------------------------------------------- |
| `MIKROSCOPE_API_ADDR`     | `--api`        | `record`, `mark`, `forward` | RouterOS binary API `host:port`; <Src path=".env.example" /> has `192.168.88.1:8728`                            |
...
| `MIKROSCOPE_INTERFACES`   | `--interfaces` | `forward`                   | comma-separated interfaces for `monitor-traffic`; <Src path=".env.example" /> has `bridge,ether1` commented out |
...
| `MIKROSCOPE_HOST_TAG`      | `--host-tag`    | `router`            | host tag on every point in every sink; <Src path=".env.example" /> has `rb5009` commented out |
...
file <Src path=".env.example" /> carries both, commented out, in its Grafana block.
...
<ScrollTable label="Memory limit against RSS and CPU per sample, RB5009, 2026-09-17" nowrap sticky={false}>
...
The rows are the ones the comment on `memLimitRingFactor` in <Src path="internal/router/options.go" /> records.
