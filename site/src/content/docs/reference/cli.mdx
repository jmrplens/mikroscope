...
description: Every verb and flag of the mikroscope CLI, with its default, its accepted range and the MIKROSCOPE_* variable that sets it, read from the Go source.
---
...
variable can set it. It is read from the Go files in <Src path="cmd/mikroscope/" />
and from <Src path="internal/router/options.go" />, not from the help text, and
...
`Finish` in <Src path="internal/router/options.go" />, one function for all
...
shows](/mikroscope/install/prerequisites/#what-the-running-agent-shows) and never change the exit
status.
...
<Aside type="note" title="Three ways to get the agent image, one flag apart">
	...
	file <Src path="cmd/mikroscope/main.go" /> gets it by whichever of three routes the flags ask for. With neither
	flag they run `go build -trimpath ./cmd/mikroscope-agent` with `CGO_ENABLED=0`, `GOOS=linux` and
	the `--arch` as `GOARCH` (<Src path="internal/image/image.go" />), which needs a checkout of the repository
...
| `--mem-limit-mb` | `0` (derived)                   | none                      | `0`, or 8–1024                                             | agent Go soft memory limit in MiB (envlist `MEM_LIMIT_MB`). `0` derives it from the ring: rate × buffer × <Measured id="ring.lineChargedBytes" /> × 2.5, rounded up, at least 16 MiB and at most ¾ of `--memory-max` while that still leaves room for the ring. That gives 16 at the defaults. A number you pass is used as is                                                                                                                                         |
...
(<Src path="internal/router/steps.go" />). `start-on-boot` has no flag of its own either: it
...
<Aside type="caution" title="Fixed in 1.1.0: the expose rules could not be removed at all">
...
<ScrollTable label="uninstall targets">
...
<Aside type="caution" title="Three sinks store nothing this can remove">
...
<Aside type="note" title="InfluxDB 3 deletes a table by renaming it">
...
sample (<Src path="internal/forward/" />); through 1.2.0 it listed the flag and ignored it.
...
| `--batch`       | `0`                          | none                  | `record`, `forward`         | samples per pull; `0` is twice what one `--poll` produces at the agent's rate, never under 20. The relay caps a pull at <Measured id="relay.maxBatch" />. A pull repeats until it comes back short                                                                                                                                                   |
...
| `--conntrack-every` | `0`     | none                    | ask the conntrack count this often; `0` never, because it is a table scan. One scan took <Measured id="api.conntrackMs" /> at <Measured id="conntrack.api" /> entries on the RB5009 (2026-09-11). `slow` sets it to `0` unless given explicitly                                                                                                                                                                                                        |
...
the file <Src path="cmd/mikroscope/sinkflags.go" />), so the bucket layout does not change when it
...
<ScrollTable label="Grafana flags of forward">
...
[Import and check](/mikroscope/dashboards/import-and-check/) says which sinks
...
package <Src path="internal/version/" />. It takes no flags.
