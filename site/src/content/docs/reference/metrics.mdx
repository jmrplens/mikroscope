...
description: Every Prometheus metric family mikroscope's collector exposes for a MikroTik RouterOS device, by source, with its type, its labels and when it is absent.
head:
  - tag: title
    content: Prometheus metric families for MikroTik RouterOS | mikroscope
---
...
from <Src path="internal/expo/expo.go" />, <Src path="internal/agent/capture.go" />
and <Src path="internal/sinks/prometheus.go" />. A family is listed under the source it
...
at (`promHistogramRateHz` in <Src path="cmd/mikroscope/sinkflags.go" />), so that its
...
<NotClaimed variant="untested">
	The last two are still arithmetic, from <Src path="internal/expo/expo.go" />: the collector's `Totals` is
...
(<Src path="internal/dashboards/panels.go" />, the disk in-flight panel). Unlike the floored
...
- [The agent's HTTP endpoints](/mikroscope/reference/http/): `/healthz`, `/capabilities`,
