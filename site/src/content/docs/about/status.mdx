...
- **`forward`**, the collector, merges the kernel tier with the RouterOS API
  ...
  hash.
- **Two Grafana dashboards**, one per store, generated from one panel list,
  with alert rules. On 2026-09-16, in Grafana 13.2.1, against the RB5009 with
...
That is above the budget of <Measured id="budget.cpu" /> of one core and <Measured id="budget.rss" /> RSS. The
...
- [The rate ceiling](/mikroscope/cost/rate-ceiling/): the five measured runs at 10, 50 and 100 Hz.
