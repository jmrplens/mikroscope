...
description: RouterOS 7.24 or later, the container package and a device-mode step that needs a hand on the router; what your host needs, and how `doctor` checks each.
head:
  - tag: title
    content: "What the router needs: RouterOS 7.24+, device-mode | mikroscope"
---
...
- **`layer2-loop`**: three or more frames in the window that came back in on a
  ...
  every <Measured id="loop.helloGap" />, the STP hello interval ([RouterOS default](https://help.mikrotik.com/docs/spaces/ROS/pages/21725254/Spanning+Tree+Protocol#SpanningTreeProtocol-Defaultvalues)) — thirty times in a 60 s ring — while every
