...
`/container/add remote-image="jmrplens/mikroscope-agent:1.0.5" …` and the plan
prints `the router pulls … (nothing is uploaded)` where the upload line would
be. The release publishes the image twice, as
`jmrplens/mikroscope-agent:1.0.5` on Docker Hub and as
`ghcr.io/jmrplens/mikroscope-agent:1.0.5` on GHCR. Both carry `linux/amd64`,
...
A reference with no host — `jmrplens/mikroscope-agent:1.0.5` — leaves the
...
pull of `jmrplens/mikroscope-agent:1.0.5` from Docker Hub at 2 ms, and the
...
1. Download `mikroscope_1.0.5_<os>_<arch>.tar.gz` (`.zip` on Windows) and the
...
<Aside type="tip" title="Two assets have similar names">
	...
	`mikroscope-agent_1.0.5_linux_arm64.tar.gz` is an archive of the bare agent binary, for reading or
