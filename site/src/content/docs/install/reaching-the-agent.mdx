...
description: The agent listens only on its veth address — how your host reaches it directly, through the RouterOS API relay or with `--expose`, and what each one costs.
---
...
`internal/version.Version`, and both the <Src path="Makefile" /> and the release take that
from the <Src path="VERSION" /> file, so an agent put there by 1.0.0 reports `1.0.0`. An agent
...
- Each call returns at most <Measured id="relay.replyMaxBytes" />; anything longer is truncated silently
  by RouterOS. The relay therefore asks for at most <Measured id="relay.maxBatch" /> samples per pull and
...
## `--expose`, on the router's LAN address {#expose-on-the-routers-lan-address}
...
| relay      | an API user with `read,api,test`; `--transport relay` or `auto` | <Measured id="relay.replyMaxBytes" /> and <Measured id="relay.maxBatch" /> samples per call, ~1 s in about half the calls, no token |
