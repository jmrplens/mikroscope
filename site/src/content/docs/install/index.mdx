...
description: A scripted round trip left the RB5009's /export byte-identical apart from its header — what `install` writes to RouterOS, in order, and how it is removed.
head:
  - tag: title
    content: Installing the agent on a MikroTik router | mikroscope
---
...
published 1.2.1 image for arm64 is <Measured id="image.size" />, almost all of it the agent binary); `uninstall` removes it again.
...
predates 1.1.0, from which `uninstall` removes only with `--yes`, and <Src path="scripts/roundtrip.sh" />
...
`all`; see [Commands and flags](/mikroscope/reference/cli/).
...
and kernel names](/mikroscope/reference/port-names/) shows how to take it. If
...
and so on); <Src path=".env.example" /> documents them. The CLI does not read `.env` itself:
