...
head:
  - tag: title
    content: "What the installer refuses on a MikroTik router | mikroscope"
---
...
empty; verified for both on <Verified of="find-quoting" />. A bare word such as `tcp` is read as a
...
- **`--remote-image <reference>`** makes the router pull the image itself, so nothing is uploaded
  ...
  example `:{{MIKROSCOPE_VERSION}}`), carries no host and leaves the setting as the router has it, so a router still at RouterOS's
  default needs it set to `https://registry-1.docker.io` first, as
  [A registry pull](/mikroscope/install/routes/#a-registry-pull) says. Trust in the
...
flags](/mikroscope/reference/cli/#pass-the-same-shape-to-status-upgrade-and-uninstall) has the
run.
