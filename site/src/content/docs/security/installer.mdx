...
- **`--remote-image <reference>`** makes the router pull the image itself, so nothing is uploaded
  ...
  `;`-joined line. It goes into `remote-image=` whole, registry host included: a reference with no
  ...
  on <Verified of="remote-image-host" />), and
  [Nothing to set on the router](/mikroscope/install/routes/#nothing-to-set-on-the-router) says
