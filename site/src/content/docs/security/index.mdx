...
| `mikroscope doctor`, `install`, `upgrade`, `uninstall`, `status` | your host, when you run them         | the router over your own admin ssh; `install`, `upgrade` and `status` also probe the agent's `/healthz`, and `doctor` reads `/healthz` and then the agent's whole ring | your ssh key; `doctor` also presents the agent's token if one is set |
...
- `install --remote-image <reference>` uploads nothing: the router itself fetches the image from the
  ...
  registry it pulls from; [what the installer refuses](/mikroscope/security/installer/) has what
