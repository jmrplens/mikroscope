...
description: Which piece of mikroscope runs on the MikroTik router and which on your host, what each reaches, where every credential lives, and how to report a flaw.
head:
  - tag: title
    content: "Security: what runs on the MikroTik router | mikroscope"
---
...
- `install --remote-image <reference>` uploads nothing: the router itself fetches the image from the
  registry the reference names, which mikroscope writes into `remote-image=` with its host,
  `registry-1.docker.io` for Docker Hub; the global `/container/config registry-url` is neither
  needed nor written. You trust that registry and the router's path to it, and mikroscope verifies
  nothing about what arrives. `/container/config` also holds one registry username and password for
  the whole device, and a Docker Hub login sent to GHCR ended a pull in `auth error`;
  [what the installer refuses](/mikroscope/security/installer/) has what that means and what
  `doctor` checks.
...
- **`privileged=yes` by default**, `--privileged=false` to opt out. The setting needs RouterOS 7.24 or later, whose [changelog](https://download.mikrotik.com/routeros/7.24/CHANGELOG) added privileged mode for containers. It drops the container's user namespace (verified on <Verified of="privileged-namespaces" />), which is what makes the kernel log, `/proc/slabinfo`,
...
The policy itself is <Src path="SECURITY.md" />. It promises an acknowledgement within a week and,
