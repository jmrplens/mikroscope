...
description: Six measured runs at 10, 20, 50 and 100 Hz on an RB5009 at its shipped configuration, none of which lost a sample, and what sampling faster buys.
head:
  - tag: title
    content: The rate ceiling on a MikroTik RB5009 | mikroscope
---
...
data costs about 6.3 times the CPU, not ten: <Measured id="run.100hz.cpu" /> at 100 Hz
against <Measured id="run.10hz.cpu" /> at 10.
...
are the ticks that slip: <Measured id="run.100hz-floor.slippedPct" /> of them at 100 Hz with every
source on every tick, against <Measured id="run.100hz.slippedPct" /> at the default floors. The CPU headroom is
larger than the timing headroom, which is why the ceiling is a statement about
I/O rather than about the A72. Even at the default floors at 100 Hz the mean read was <Measured id="read.100hz.meanMs" /> but the
worst <Measured id="read.100hz.worstMs" />, and a tick whose read outlasts its period is late by definition. That, and not a
CPU wall at <Measured id="run.100hz.cpu" /> of one core, is where the few slips there come from.
...
<NotClaimed>
	...
	device](/mikroscope/cost/#measuring-it-on-your-own-device) lists.
</NotClaimed>
