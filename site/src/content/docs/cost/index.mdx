...
description: What the agent costs a MikroTik RB5009 at 10 to 100 Hz, measured from its own cgroup, and how to measure it on a device that is not this one.
head:
  - tag: title
    content: The cost of the observer on a MikroTik RB5009 | mikroscope
---
...
At the install default — 10 Hz, the default per-source floors, a 60 s ring — the agent costs a
MikroTik RB5009 **<Measured id="run.10hz.cpu" /> of one core and <Measured id="run.10hz.rss" /> RSS**,
read from its own cgroup:

<Provenance of="rates-2026-09-18" />

An observer that costs 20 % of the thing it observes is not measuring the
router, it is measuring itself. So that number is a first-class result here, not
a footnote: the agent reports it on every sample, and CI asserts the image-size budget.

## The budget, and what it actually costs

The budget is **≤ <Measured id="budget.cpu" /> of one core, ≤ <Measured id="budget.rss" /> RSS,
≤ <Measured id="budget.image" /> image**. The published 1.2.1 arm64 image is <Measured id="image.size" /> (6 690 304 B, measured on
the release asset on 2026-09-24); the 1.0.0 image was <Measured id="image.size.v100" />, the figure
...
a number: [the rate ceiling](/mikroscope/cost/rate-ceiling/) is that table.
