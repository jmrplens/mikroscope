---
title: Prometheus
description: The collector’s own `/metrics` — every agent family recomputed from the samples, plus the API tier, the derive stage and the collector’s counters — and the two scrape jobs that go with it.
---
...
- [Import and check](/mikroscope/dashboards/import-and-check/): the dashboard these two scrape jobs
