...
description: On an RB5009, RouterOS cpu-load fitted a ~1 s trailing mean of /proc/stat. What else the collector asks the binary API, and the presets that ask less.
head:
  - tag: title
    content: The RouterOS API tier and what cpu-load averages | mikroscope
---
...
7.24.2 it tracked a trailing mean of **<Measured id="cpuLoad.window1" /> that reached
the API <Measured id="cpuLoad.delay1" /> late** in one hour and **a mean of <Measured id="cpuLoad.window2" />, <Measured id="cpuLoad.delay2" />
late** in another, each the best fit against the kernel's own `/proc/stat` busy ratio
read at 10 Hz on 2026-09-15 (r = <Measured id="cpuLoad.r1" /> over the hour's <Measured id="cpuLoad.samples1" />
API samples, and r = <Measured id="cpuLoad.r2" /> over the other's <Measured id="cpuLoad.samples2" />).
...
cadence: <Measured id="api.conntrackMs" /> at <Measured id="conntrack.api" /> entries on the RB5009 (2026-09-11, the day before the slab readings; the time of day is not recorded), a single reading with no spread. It is off by default because it
...
said <Measured id="conntrack.api" /> the day before the agent's slab said <Measured id="conntrack.slab" />,
...
- [Conntrack without the API](/mikroscope/playbooks/conntrack/): reading the connection count from
  the slab instead.
- [Compared with the alternatives](/mikroscope/start/compared/): what SNMP, the RouterOS API and
