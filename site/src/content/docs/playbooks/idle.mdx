...
description: "Sixty idle seconds on a MikroTik RB5009 read at 10 Hz: the busy floor, a softnet squeeze that is never zero, and a kernel log that stays silent."
head:
  - tag: title
    content: "An idle MikroTik RB5009, per core, at 10 Hz | mikroscope"
---
...
the same device, over 3 738 704 per-CPU samples: about <Measured id="squeeze.backgroundOne" /> of
samples carry one squeeze as background, <Measured id="squeeze.backgroundTwo" /> exactly two,
and <Measured id="squeeze.exactlyThree" /> exactly three. Measured on this router on 2026-09-15: a rule
