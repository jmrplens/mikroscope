...
description: "Eight readings of kernel and port data from a MikroTik RB5009: two production faults, three provoked events, two unprovoked ones and the idle baseline."
head:
  - tag: title
    content: "Reading MikroTik RouterOS faults in kernel data | mikroscope"
---
...
[A port losing frames](/mikroscope/playbooks/port-errors/) is a separate campaign
...
<ScrollTable label="The eight readings and their signatures">
...
| [A loop only the kernel could see](/mikroscope/playbooks/loop/)      | real, found by accident on the production router | `events` (the `kmsg` source)                   | `events` at <Measured id="loop.eventsBefore" />, <Measured id="loop.helloGap" /> apart; RouterOS's log and port monitor reported a healthy device |
...
| [A port losing frames](/mikroscope/playbooks/port-errors/)           | real, found on the production router 2026-09-19  | the API tier's per-port MAC counters           | one port at 0.5 % of packets lost to microbursts, then none                                                               |
...
it, are under `mikroscope-wakeup-storm` in [Alert rules](/mikroscope/dashboards/alerts/).
