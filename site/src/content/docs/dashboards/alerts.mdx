...
- **A collector without the API tier.** The port-error, dark-bridge-port and egress-queue rules read
  ...
  of which are OK (the PostgreSQL dark-port rule fails either way; see [The files](#the-files)). On
...
<NotClaimed variant="untested" title="What has not been tried">
	**A rule resolving.** The loop on the reference device ended on 2026-09-23, but the return of
	...
	dark-port one cannot run; see [The files](#the-files)). **The Prometheus and PostgreSQL forms**,
