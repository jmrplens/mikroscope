...
description: Binding the dashboards to a Grafana datasource, what import and check ask the store first, and what check proves against a live Grafana and what it cannot.
---
...
carries <DashboardCount store="postgres" of="panels" /> panels against InfluxDB's <DashboardCount store="influxdb" of="panels" />.
...
`mikroscope-<store>.json` from <Src path="dashboards/" /> that matches your datasource (influxdb,
...
cannot produce (three on Graphite and Elasticsearch) sit in the not-available row with their
queries switched off, and every other panel ships with its query on, whether your store holds its
measurement or not. On InfluxDB a panel whose table or column is missing then shows InfluxDB 3's
...
[the annotations section](/mikroscope/dashboards/#the-red-dashed-lines-on-every-panel) describes.
...
and the file <Src path="internal/dashboards/grafana.go" /> has one for those two and
...
through `/api/ds/query`, returned 44 rows over 6 hours. The same day, Grafana 12.3.0 and 13.2.1 over
a throwaway InfluxDB 3.11.2 Core store were used for the renders on
[Five dashboards](/mikroscope/dashboards/#not-available-on-this-device): the not-available row, a
