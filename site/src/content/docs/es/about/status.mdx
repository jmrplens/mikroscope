...
- **`forward --grafana`** concilia una fuente de datos y un dashboard por cada
  store al que escribe el colector, al arrancar, antes de la primera muestra. Se
  ha ejecutado contra el Grafana del dueño (2026-09-19, InfluxDB 3) y contra el
  de la batería de contenedores para los cinco stores (2026-09-20), donde
  después se preguntó cada panel de cada dashboard por la propia API de Grafana
  **contra la fuente de datos que el colector había construido**. Tres stores
...
funcionando de forma continua desde el 2026-09-19. Las mediciones no se han
repetido sobre 7.24.4.
...
<NotClaimed variant="untested">
	...
	hardware. Ningún host RouterOS x86_64. Ningún RouterOS anterior a 7.24, y ninguna de las cifras
	tomadas en el equipo vuelta a medir sobre 7.24.4. Nada que necesite un reinicio, que espera a una
...
`VERSION` es **1.1.0**, compilada en los dos binarios y lo que devuelve
`mikroscope version`. Dos números intermedios no tienen release propia: la 1.0.8
y la 1.0.10 se prepararon y nunca se etiquetaron, así que la v1.0.9 lleva los
cambios de la 1.0.8 y la v1.1.0 los de la 1.0.10. Un número que este proyecto se
salta es uno que nadie puede instalar, y por eso el changelog no tiene sección
para ninguno de los dos. Una etiqueta `v*` lanza la configuración de GoReleaser,
que publica:
...
- **Tar de imagen del agente para cargar a mano**, uno por arquitectura —
  ...
  router.
- **La imagen del agente en dos registros**, `jmrplens/mikroscope-agent:1.0.9`
  en Docker Hub y `ghcr.io/jmrplens/mikroscope-agent:1.0.9` en GHCR, cada una un
