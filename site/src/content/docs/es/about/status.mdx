...
Todo lo siguiente, salvo la batería de pruebas y los siete destinos señalados
...
- **La CLI de despliegue** lo instala, lo actualiza y lo retira — `doctor`,
  ...
  con un tiempo de ida y vuelta de 5–7 ms.
...
- **`forward`**, el colector, une la capa del kernel con la capa de la API de
  ...
  ejecutado desde el RB5009; Loki, OTLP, Graphite, Elasticsearch, SQL, Telegraf
  y stdout solo los ejercita byte a byte la batería de extremo a extremo,
  contra simulaciones en el equipo de desarrollo. Su etapa de derivación
  añade valores derivados y eventos de detección junto a las filas en bruto, y
  emite un flujo de datos del equipo una vez por cada hash de capacidades.
- **Dos paneles de Grafana**, uno por almacén, generados a partir de una sola
  lista de paneles, con reglas de alerta. El 2026-09-16, en Grafana 13.2.1,
...
- **Tar de imagen del agente para cargar a mano**, uno por arquitectura —
  ...
  router.
- **La imagen del agente en GHCR**, `ghcr.io/jmrplens/mikroscope-agent:1.0.0`,
  un único manifiesto sobre `linux/amd64`, `linux/arm64` y `linux/arm/v7`, que
  `install --remote-image` hace descargar al router.
