...
`site.webmanifest`. El enlace al `.ico` dice `sizes="32x32"`, lo que mantiene a Chromium solo en
el SVG: Chromium 153 sin interfaz pidió únicamente `favicon.svg` el 2026-09-25. El manifiesto
nombra `icon-192.png`, `icon-512.png` e `icon-maskable-512.png` con URL relativas a él mismo, y no
lleva `id`: uno relativo se resuelve contra la raíz del dominio, que es la dirección de otro
sitio. Deja fuera `favicon.svg`, el único icono sin fondo propio, porque lo que acaba en una
pantalla de inicio lleva el suyo. Dos etiquetas `theme-color`, una por `prefers-color-scheme`,
dan el color de la propia cabecera, `#151c20` en oscuro y `#fafbfb` en claro: Chrome lee las
etiquetas, y Safari 26 las ignora y toma el color de la cabecera fija, así que los dos coinciden.
