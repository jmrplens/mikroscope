...
defecto en la barra de conmutadores, y funciona en tres de ellos: en InfluxDB y
PostgreSQL lee las filas de `mikroscope_trigger`, en Prometheus el
`mikroscope_trigger_fired_total` del colector. Los dashboards de Elasticsearch y
Graphite llevan esa misma expresión PromQL contra su propio datasource, así que su
anotación `triggers`, igual que la de `detections`, no devuelve nada. La captura en
