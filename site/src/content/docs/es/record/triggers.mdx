...
muestras. El anillo ya guarda los últimos 60 s por defecto, así que conservar
los segundos anteriores a un disparo no cuesta nada; los posteriores solo
cuestan la espera. La ventana de captura por defecto son 5 s a cada lado, así
que cabe con holgura; un `CAPTURE_PRE_S` mayor de lo que el anillo guarda es
una ventana que el anillo no puede dar, y `--buffer` es lo que la sube.
...
línea. La línea media medida en el RB5009 (RouterOS 7.24.2, 10 Hz, privileged, todas las fuentes incluido el
PMU, 2026-09-17) fue de <Measured id="ring.lineBytes" />;
...
puede leer el `memory.max` del contenedor y unos `rate × buffer ×` <Measured id="ring.lineChargedBytes" />
más `CAPTURE_MB` lo superan, se niega a arrancar, nombrando los tres ajustes que bajar o
`--memory-max` para subir. También avisa si el doble de eso supera su límite blando de memoria de
Go, `MEM_LIMIT_MB` (de 8 a 1 024 MiB). El valor por defecto del propio agente para `MEM_LIMIT_MB`
es 14; `install` lo deriva en cambio del anillo (cadencia × buffer × línea, × 2,5, como mínimo 16
y como mucho tres cuartos de `--memory-max`), lo que escribe 16 con los valores por defecto, y
`--mem-limit-mb` lo fija a mano. Esa derivación no cuenta `CAPTURE_MB`, así que un presupuesto de
capturas grande puede seguir provocando el aviso.
...
El transporte relay tira a través de `/tool fetch`, que devuelve como mucho <Measured id="relay.replyMaxBytes" /> y no envía token. Una captura con los valores por defecto ocupa unos 330 kB, cinco veces el tope de la petición. Trae las capturas
...
exposición Prometheus del colector, un documento `trigger` en Elasticsearch,
`trigger.<cause>` en Graphite, una línea `source="trigger"` en Loki, la suma
`mikroscope.trigger.fired{cause}` en OTLP y la propia línea en el destino de fichero.
Los cinco dashboards de Grafana llevan una anotación `triggers`, desactivada por
defecto en la barra de conmutadores, junto a una `detections` que está activada, y
cada una lee su propio almacén: en InfluxDB y PostgreSQL las filas de
`mikroscope_trigger`, en Prometheus el `mikroscope_trigger_fired_total` del colector.
En Elasticsearch es un filtro Lucene, `kind:trigger AND host.keyword:$host`, que marca
cada documento `trigger` con su `field` como texto y su `cause` como etiqueta
(`kind:detection`, `message` y `rule` para las detecciones). En Graphite es
`aliasByNode($prefix.$host.trigger.*, 3)`, un marcador por punto titulado con la
causa, sin texto, porque Graphite no guarda cadenas (`detection.*` y la regla para las
detecciones). Hasta 1.2.0 estos dos dashboards llevaban en su lugar la expresión de
Prometheus y no mostraban nada. Las formas de Elasticsearch y Graphite solo se han
comprobado como JSON generado; no se han ejecutado en un Grafana real. La captura en
sí se queda en el agente, en `/captures/<id>`.
...
El `/metrics` del colector lleva las familias que dicen cuánto no vieron las
capturas, construidas a partir de los contadores de `GET /sampler` del agente, que el colector lee al
arrancar y cada minuto. Cada par de condición y motivo se expone desde el principio, a 0 hasta
...
[Prometheus](/mikroscope/es/sinks/prometheus/) tiene el detalle.
...
- [Familias de métricas de Prometheus](/mikroscope/es/reference/metrics/): las
  familias de disparo y captura con todas las demás familias de la exposición del colector.
