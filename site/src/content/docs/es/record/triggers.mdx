...
Los cinco paneles de Grafana llevan una anotación `triggers`, desactivada por
defecto en la barra de conmutadores: en los almacenes SQL lee las filas de
`mikroscope_trigger`, en Prometheus el `mikroscope_trigger_fired_total` del
colector. La captura en sí se queda en el agente, en `/captures/<id>`.
...
capturas, construidas a partir de los contadores de `GET /sampler` del agente. Cada par de condición y motivo se expone desde el principio, a 0 hasta
