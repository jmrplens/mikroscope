...
- **relay** ejecuta `/tool fetch output=user` en el router por la API binaria,
  ...
  limitada a <Measured id="relay.replyMaxBytes" />, así que una petición por relay pide como mucho 18
...
<Aside type="caution" title="Una petición por relay llena aún puede llegar al límite">
	...
	de fetch de <Measured id="relay.replyMaxBytes" />, una línea media de 2 560 B y un margen del
	134 %; la media son los <Measured id="ring.lineBytes" /> medidos en el RB5009 (RouterOS 7.24.2,
...
<Aside type="caution" title="Marca mientras corre la grabación, no después">
	`record` mantiene abierto `<out>.markers.csv` en modo añadir, así que `mark --out <prefix> "text"`
	desde un segundo terminal añade su fila sin riesgo mientras la grabación sigue corriendo, y la
	siguiente escritura del grabador — una nota tecleada o un hueco — cae detrás de ella, no encima.
