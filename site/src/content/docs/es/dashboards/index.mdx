...
description: "Cinco dashboards de Grafana para telemetría de kernel de MikroTik RouterOS, uno por almacén y de una sola lista: cada sección y panel, y por qué difieren."
head:
  - tag: title
    content: Dashboards de Grafana para MikroTik RouterOS | mikroscope
---
...
El directorio <Src path="dashboards/" /> contiene un dashboard de Grafana por almacén para la
telemetría de un equipo MikroTik RouterOS —cinco— todos generados por `mikroscope dashboards gen`
a partir de una única lista de paneles en <Src path="internal/dashboards/" />. Esta página
responde a qué hay en ellos: qué secciones, qué paneles en cada una, qué muestra el Overview
antes de desplegar nada y qué paneles no puede llevar cada almacén. Importarlos y
...
```sh
mikroscope dashboards gen                    # escribe los ocho ficheros en ./dashboards
...
`mikroscope-<almacén>`, uno por dashboard, así que una reimportación actualiza el mismo dashboard en
su sitio en vez de crear un segundo. Un test comprueba que dos generaciones del dashboard de
InfluxDB son idénticas; regenerar el 2026-09-15 reprodujo byte a byte los ficheros que había ese día
en el repositorio, y los añadidos después no se han vuelto a comprobar así.
...
dashboard de Prometheus consulta un Prometheus que hace scrape del `/metrics` del colector, y de
nada más: lleva el tier de kernel recalculado a partir de las muestras, las familias derivadas y de
detección propias del colector y —desde 1.0.5, cuando el agente dejó de servir exposición— lo que
solo el muestreador puede producir, que ahora llega al colector como dato. Prometheus 3 representa
el borde del bucket cero de un histograma como `le="0.0"`, así que una consulta que lee ese bucket
casa con `le=~"0|0.0"`. El trabajo de scrape está en
...
<NotClaimed title="Esto es una base de demostración, no un router">
	...
	por el paso de llenado de <Src path="test/e2e/docker/" /> y fotografiada por el
	script <Src path="site/scripts/gen-dashboard-captures.mjs" />. Nada en ellas viene de un equipo real, el host se llama
...
despliega, así que el primer renderizado pide al almacén los trece paneles del Overview (doce en PostgreSQL) y no los <DashboardCount store="influxdb" of="panels" />.
...
average (1 min) against the core count", "Reboots in the window", "Detections in the window" y
"Port errors in the window" viven solo aquí, igual que "OOM kills in the window" en Prometheus,
donde la copia de la sección de recuperación de memoria no tiene PromQL y se quita. Las copias comparten su SQL; en Prometheus, el
...
1. **CPU busy per core**, **Memory in use, against the kernel's own total** (
   `(MemTotal − MemAvailable) / MemTotal` en el tiempo, con líneas discontinuas al 75 % y al 90 %) y **Connections tracked
...
5. **Port errors in the window** (todos los errores MAC tipificados de todos los puertos, sumados;
   capa de la API, en blanco con `--api-mode off`) y **Ticks never delivered, this window**, que
   cuenta además los reinicios del agente.
...
<NotClaimed variant="not-measured" title="Una pantalla, medida una vez">
	...
	1 080 px — una pantalla de escritorio sin nada cortado. Ahora el Overview tiene trece paneles en
	InfluxDB y Prometheus, donde "Port errors in the window" y "Ticks never delivered" comparten la
	última línea, y doce en PostgreSQL, donde "Ticks never delivered" ocupa esa línea él solo; esa
	altura no se ha medido en un navegador, aunque el JSON de esos tres dashboards que hay en el
	repositorio le da 31 unidades de cuadrícula, cabecera de fila incluida (21 en Graphite, 14 en
	Elasticsearch). En un móvil no es una pantalla: Grafana apila una fila de 24 columnas en una sola
	columna por debajo de unos 768 px, y un Overview de ocho paneles se renderizó con 2 188 px de
	alto en una ventana de 844 px el 2026-09-12. El 2026-09-24 el dashboard de InfluxDB se importó en
	el Grafana de producción y se capturó a 390x844 y a 1600x1000 para comprobar el tamaño fijo de los
	paneles stat, y los números se leían a un tamaño normal en ambos; ese día no se midió la altura del
	Overview.
</NotClaimed>
...
En "Trigger fires and suppressions per bin", las supresiones solo están en Prometheus: las
representa el colector en su exposición y ningún almacén lleva un campo para ellas.
...
<ScrollTable label="Las dos capas de anotaciones">
...
[capturas disparadas](/mikroscope/es/record/triggers/).

**Antes de la primera detección.** Un almacén InfluxDB no tiene la tabla `mikroscope_detection`
hasta que el colector escribe su primera detección. InfluxDB 3 rechaza una consulta que nombra una
tabla que no existe, y ninguna forma de la consulta lo evita: `WHERE false`, un `UNION ALL` y una
guarda `EXISTS` sobre `information_schema` fallaron todas al planificar en InfluxDB 3.11.2 Core el
2026-09-25. `mikroscope dashboards import` pregunta antes al almacén y, si la tabla no está, publica
la capa de detecciones apagada, con su consulta, para que puedas encenderla más adelante. Los
ficheros del repositorio y una [importación a mano](/mikroscope/es/dashboards/import-and-check/#importar-a-mano)
no tienen a quién preguntar, así que la capa sigue encendida. La consulta falla entonces en cada
carga y cada refresco, pero en silencio: el 2026-09-25, sobre InfluxDB 3.11.2 Core, Grafana 13.2.1
no mostró nada por ello en el dashboard y escribió cada vez una línea de nivel error
`Partial data response error` en su log. Las marcas aparecen solas cuando la primera detección crea
la tabla. En Prometheus el contador simplemente no existe y la consulta devuelve un resultado vacío,
así que la capa sigue encendida también tras un sondeo. El sink de PostgreSQL crea todas las tablas
antes de su primera inserción, así que allí la consulta devuelve cero filas: el 2026-09-25, en la
batería de contenedores del proyecto (Grafana 13.2.1, PostgreSQL 18.6), la consulta de la capa
respondió 200 sin filas sobre una ventana anterior a cualquier detección, y 10 filas sobre la de la
propia ejecución. Fue una consulta por la API de Grafana, no un dashboard renderizado.

<NotClaimed variant="untested" title="La capa de detecciones en Grafana 12.3.0">
	En esos mismos renderizados, Grafana 12.3.0 no llegó a enviar la consulta de la capa de
	detecciones de InfluxDB, con la tabla o sin ella: su interruptor se quedó con un indicador de
	carga, apagarlo, encenderlo y refrescar no envió nada, y no se dibujó ninguna marca, ni en el
	dashboard completo ni en una copia de un solo panel. Grafana 13.2.1 la envió en el acto sobre el
	mismo almacén. Por qué, y si 12.3.2 hace lo mismo, no se ha investigado.
</NotClaimed>
...
- **InfluxDB y PostgreSQL**: una fila de `mikroscope_detection` o `mikroscope_trigger` por marca,
  con su mensaje.
- **Prometheus**: `increase(…[1m]) > 0` con un paso de 1 minuto, así que allí una marca es el
  minuto, no el instante.
- **Elasticsearch**: un filtro Lucene, `kind:detection AND host.keyword:$host` (o `kind:trigger`),
  sobre los documentos de suceso. El texto de una detección es su `message` y su etiqueta la `rule`;
  un documento de disparo no lleva mensaje, así que su texto es el `field` que cruzó y su etiqueta la
  `cause`.
- **Graphite**: `aliasByNode($prefix.$host.detection.*, 3)` (o `.trigger.*`), el punto por suceso
  que escribe el sink. Grafana pone una marca en cada punto no nulo y la titula con el nombre de la
  serie, que es la regla o la causa. No hay mensaje, porque Graphite solo guarda números, y la marca
  cae en el instante del punto que Grafana recibe: Grafana pide como mucho 100 puntos, así que
  varios sucesos muy juntos pueden volver como uno.

Las capas de Elasticsearch y Graphite las comprueban los tests unitarios contra lo que escribe cada
sink; no se han ejecutado en Grafana, porque la comprobación de importación solo recorre paneles.
...
<Capture
	name="annotations"
	alt="La sección de vista general del dashboard de InfluxDB con la capa de anotaciones de detecciones activada: rayas verticales rojas discontinuas, cada una con un triángulo al pie del eje de tiempo, cruzan todos los paneles en el instante de una detección."
	caption="La capa de detecciones, activada: una raya roja discontinua por detección, en todos los paneles a la vez."
/>
...
de referencia (RouterOS 7.24.2, kernel 5.6.3 arm64) no puede producir. En Graphite y Elasticsearch
contiene tres: los paneles de profundidad de cola y de porcentaje ocupado no tienen consulta en el
lenguaje de ninguno de los dos almacenes, así que esos dos dashboards los dejan fuera.
...
todos los dispositivos listados se quedan a cero, así que ningún destino llega a crear la tabla. El
almacén de referencia seguía sin ninguna de las dos tablas el 2026-09-25, con RouterOS 7.24.4.
...
lugar de datos. Hasta 1.3.0 las consultas se ejecutaban al abrir la fila: el 2026-09-25, con el
fichero de InfluxDB del repositorio importado en Grafana 12.3.0 y 13.2.1 sobre un almacén InfluxDB
3.11.2 Core sin ninguna de las dos tablas, abrir la fila envió las consultas de los cinco paneles,
cada una respondida con `table … not found`, y pintó cinco insignias rojas. Con las consultas
apagadas no envió ninguna ni pintó insignias, en ninguna de las dos versiones, ni en 13.2.2 en un
segundo renderizado ese mismo día. Si tu almacén sí tiene la medida, vuelve a encender las
consultas del panel en su editor, o ejecuta `import` (InfluxDB y Prometheus; los otros tres no se
pueden sondear), que pregunta al almacén y devuelve el panel a su propia sección.

**En los otros cuatro almacenes la fila conserva sus consultas encendidas**, como hasta 1.3.0,
porque allí que falte una medida no es un error. El sink de PostgreSQL crea todas las tablas,
`mikroscope_psi` y `mikroscope_disk` incluidas, antes de su primera inserción, así que las
consultas devuelven cero filas; Prometheus responde a una métrica ausente con un resultado vacío, y
Graphite a una ruta que no casa con nada con un cuerpo vacío. Elasticsearch responde con una línea
en cero: una `sum` sobre documentos que no tienen el campo da 0 en cada intervalo. Cada uno de esos
casos se midió el 2026-09-25 enviando las consultas de los ficheros del repositorio a través de
Grafana 13.2.1 en la batería de contenedores del proyecto, con el almacén sin nada para ellas
(Prometheus 3.14.0, PostgreSQL 18.6, graphite-statsd 1.1.10-5, Elasticsearch 9.5.3): todas
respondieron 200 sin error, y en Elasticsearch todos los puntos fueron 0, tanto si otro router del
mismo índice había mapeado el campo como si no. Ese cero es un valor, no un resultado vacío, y no
distingue un router que no tiene la medida de uno cuyos valores suman cero: es el título de la
fila, no la línea, lo que dice que la medida no se produce. Las
consultas siguen encendidas para que un router que sí produce la medida llene el panel sin tocar
nada; en PostgreSQL, Graphite y Elasticsearch ningún sondeo podría volver a encenderlas. Tras un
sondeo, en cualquier almacén, un panel que el sondeo no encontró se publica con sus consultas
apagadas, como en InfluxDB.
...
sitio, con las consultas encendidas, sin tocar el generador. Eso requiere un almacén que `import`
pueda sondear, InfluxDB o Prometheus; los otros tres no se pueden sondear y conservan la fila
compilada. `import` también funciona en el otro
sentido: en un almacén al que le falta una medida que sí tenía el equipo de referencia — o un campo
añadido después de que ese almacén se escribiera por primera vez — el panel pasa a esta fila con sus
consultas apagadas, así que muestra su explicación y no una insignia roja de error. Cómo decide el
sondeo está en [Importar y comprobar](/mikroscope/es/dashboards/import-and-check/#el-sondeo).
...
- **Sus números stat tienen un tamaño fijo.** Un panel stat dibuja su valor a 32 px y su título a
  16 px en vez de con el tamaño automático de Grafana. El tamaño automático llena el panel, y en un
  móvil cada panel ocupa todo el ancho a su propia altura: en el dashboard de producción a 390x844,
  el 2026-09-24, un simple "0" se dibujaba de unos 60 px de alto y cada panel stat ocupaba cerca de
...
<NotClaimed variant="untested" title="Versiones de Grafana">
	Los dashboards se comprobaron en Grafana 12.3.2 (2026-09-12), Grafana 13.2.1 (pasadas en el
	navegador el 2026-09-12 y el 2026-09-14, `check` y renderizado el 2026-09-15, y `check` otra vez el
	2026-09-16) y Grafana 12.3.0 (2026-09-21: `import` contra el almacén InfluxDB 3 en producción y
	después `check` sobre una ventana de 15 min —167 paneles devolvieron filas, los 9 que se sabe que
	salen vacíos salieron vacíos, y el marcador de detecciones del Overview salió vacío porque el
	almacén no tenía ninguna detección en esa ventana, que es el único caso que `check` señala y que
	un navegador lee como el «none» sano—). El 2026-09-24 el dashboard de InfluxDB se importó en el
	Grafana de producción, cuyo endpoint de salud informaba de la 13.2.2 al leerlo más tarde ese mismo
	día, y se renderizó a 390x844 y a 1600x1000 para comprobar el texto de 32 px de los paneles stat y
	la serie temporal de memoria; no se hizo un recuento de insignias fila a fila. Las opciones de los
	...
	probado ninguna versión aparte de esas cuatro. En la 12.3.0 los únicos renderizados son los del
	2026-09-25 sobre un almacén InfluxDB 3.11.2 Core desechable, de la fila no disponible y de la capa
	de detecciones (más arriba); el resto del dashboard se comprobó allí solo por consulta.
</NotClaimed>
