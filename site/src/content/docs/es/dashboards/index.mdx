...
antes de su primera inserción, así que allí la consulta devuelve cero filas; eso sale del código, no
de un dashboard renderizado.
...
**Todas las consultas de esta fila se publican apagadas** (ocultas, en el editor de consultas de
Grafana), así que abrir la fila no ejecuta nada y cada panel muestra una nota breve en lugar de
datos. Hasta 1.3.0 las consultas se ejecutaban al abrir la fila: el 2026-09-25, con el fichero de
InfluxDB del repositorio importado en Grafana 12.3.0 y 13.2.1 sobre un almacén InfluxDB 3.11.2 Core
sin ninguna de las dos tablas, abrir la fila envió las consultas de los cinco paneles, cada una
respondida con `table … not found`, y pintó cinco insignias rojas. Con las consultas apagadas no
envió ninguna ni pintó insignias, en ninguna de las dos versiones. Si tu almacén sí tiene la medida, vuelve a encender
las consultas del panel en su editor, o deja que lo haga `import`.
...
sitio, con las consultas encendidas, sin tocar el generador. `import` también funciona en el otro
