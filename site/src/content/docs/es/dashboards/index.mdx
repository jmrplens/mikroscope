...
no PromQL, así que solo existen en InfluxDB. Las supresiones de disparadores viven solo en el
`/metrics` del colector; los histogramas de temporización del muestreador y el presupuesto de
capturas tienen además forma en InfluxDB. La
...
<NotClaimed variant="not-measured" title="Una pantalla, medida una vez">
	...
	medido en un navegador, aunque el JSON de los cinco dashboards que hay en el repositorio le da 31 unidades de
...
<NotClaimed variant="untested" title="Versiones de Grafana">
	...
	un navegador lee como el «none» sano—). Las opciones de los paneles están escritas según el
	esquema que espera Grafana 13.2.1 — la marca del xychart, por ejemplo, cambió de sitio entre
	Grafana 11 y 13 — y `__requires` declara Grafana 11.0.0. No se ha probado ninguna versión aparte
	de esas tres, y la 12.3.0 se comprobó solo por consulta, sin renderizar nunca en un navegador.
</NotClaimed>
