...
reservadas. Quince paneles no se reescriben y **se caen en silencio**, en vez de pasar a la fila de «no
disponible»: los del log del kernel, porque el esquema SQL no tiene tabla de recuentos para él, y
los de `mikroscope_buddy` y los contadores de interfaz de RouterOS, anchos en InfluxDB y largos en
SQL, donde un pivote es otra pregunta. El dashboard de PostgreSQL lleva por tanto 160 paneles frente
a los 175 de InfluxDB.
...
de datos, y `internal/dashboards/grafana.go` tiene una para esos dos y `cannot probe a "…"
