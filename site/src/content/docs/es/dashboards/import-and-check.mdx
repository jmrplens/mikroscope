...
| `--grafana-datasource-uid` | vacío        | adoptar una fuente de datos existente por uid en lugar de crear una                         |
...
### Dos de los cinco sinks saben describir su propia fuente de datos

Son los dos cuya dirección de escritura **es** la dirección que consulta Grafana.

<ScrollTable label="qué sinks pueden describir una fuente de datos de Grafana">

| Sink         | Fuente de datos | Por qué                                                                       |
| ------------ | --------------- | ------------------------------------------------------------------------------ |
| `--influx`   | creada          | el servidor al que escribe es el que consulta Grafana; `--influx-db` nombra la base de datos |
| `--elastic`  | creada          | la URL base a la que hace `_bulk` es la que consulta Grafana; `--elastic-index` da el patrón de índice |
| `--prom`     | adoptar         | **sirve** `/metrics` y lo raspan; el Prometheus que consulta Grafana es uno del que él no ha oído hablar |
| `--sql`      | adoptar         | escribe sentencias a un fichero, no a un servidor                              |
| `--graphite` | adoptar         | habla el puerto de ingesta de carbon, que no es la API que consulta Grafana    |

</ScrollTable>

Los tres marcados como *adoptar* necesitan `--grafana-datasource-uid`, y lo dicen por su nombre si
lo omites. Una fuente de datos adoptada es de otro y la describe otro, así que tampoco se corrige:
conciliar una que no hizo él sobrescribiría ajustes que nadie le pidió tener.
