...
description: "Una inundación ICMP de 10 s contra un MikroTik RB5009, leída desde su contenedor: la interrupción del switch como contador de puerto, y time_squeeze."
head:
  - tag: title
    content: "Una inundación de paquetes vista desde RouterOS | mikroscope"
---
...
**En resumen:** desde dentro del contenedor, una inundación dirigida al router se
ve como una línea de interrupción que se multiplica, aquí `switch0` de 5,5 k a
34 k por tramo de 5 s, con su coste en el único núcleo al que está fijada esa
interrupción, y con los softirq y `time_squeeze` subiendo a la vez. El contenedor
no ve los contadores de las interfaces del router, así que la interrupción hace
sus veces: da el inicio, el final y el núcleo que pagó, no los bytes.
...
En tramos de 5 segundos, antes y durante. No hay gráfico de esta inundación: se
provocó el 2026-09-12, antes de que empiece el historial del almacén InfluxDB de
referencia, el 2026-09-19.
...
- El squeeze no tiene un presupuesto entre el que dividirlo: `/proc/sys/net/core/*`
  ...
  contenedor.
- Un descarte, a diferencia de un squeeze, es un suceso que un router sano no
  produce: `mikroscope doctor` lo informa como `softnet-drops` cuando el anillo
  del agente en marcha contiene alguno, y la alerta `mikroscope-softnet-drops`
  salta con el primero. Ninguno de los dos cuenta squeezes.
