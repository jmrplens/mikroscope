...
description: "Ocho lecturas de datos del kernel y de puertos de un MikroTik RB5009: dos fallos de producción, tres provocados, dos sin provocar y la base en reposo."
head:
  - tag: title
    content: "Leer fallos de MikroTik RouterOS en datos del kernel | mikroscope"
---
...
es una forma, leída en datos que el agente (o, para los errores de puerto, el
tier de API) recogió en el router de referencia, con las órdenes que la
produjeron para que puedas reproducirla en tu propio equipo.

**En resumen:** cada página de abajo lee un fallo como un cambio frente a la
forma en reposo, como un log del kernel que normalmente calla y trae un flujo
constante, un núcleo al máximo mientras el total del equipo sigue moderado, o una
línea de interrupción que se multiplica. Tres de ellas llevan un gráfico sacado
del almacén InfluxDB de referencia, cuyo historial empieza el 2026-09-19: la
vuelta del bucle, la comprobación cruzada de conntrack y el puerto que pierde
tramas. Las lecturas del 2026-09-12 son anteriores a ese almacén y son tablas.

Todos los números de estas páginas, salvo los de la página de errores de puerto,
salen de una sola campaña en el router de referencia, un kernel aarch64 en una
placa con 1 GB de RAM:

<Provenance of="playbooks-2026-09-12" />

[Un puerto perdiendo tramas](/mikroscope/es/playbooks/port-errors/) es una campaña
aparte, una semana después, leída en los contadores de puerto del tier de API y
no en el agente, y su procedencia está en esa página. Cuando otra página añade una
cifra de otra fecha, lo dice junto a la cifra. Cuando un
...
<ScrollTable label="Las ocho lecturas y sus firmas">
...
| [Un bucle que solo veía el kernel](/mikroscope/es/playbooks/loop/)               | real, encontrado por casualidad en el router de producción | `events` (la fuente `kmsg`)                           | `events` a <Measured id="loop.eventsBefore" />, separados <Measured id="loop.helloGap" />; el log de RouterOS y su monitor de puertos informaban de un equipo sano |
...
| [Un puerto perdiendo tramas](/mikroscope/es/playbooks/port-errors/)              | real, encontrado en el router de producción el 2026-09-19  | los contadores MAC por puerto de la capa de la API    | un puerto al 0,5 % de paquetes perdidos por microrráfagas, y luego ninguno                                                    |

</ScrollTable>

La tormenta de despertares del 2026-09-23, un tercer fallo real en el router de
referencia, aún no tiene lectura propia: su firma medida, y la regla que la
vigila, están en `mikroscope-wakeup-storm`, en [Reglas de
alerta](/mikroscope/es/dashboards/alerts/).
...
entregar ~600 líneas ya codificadas, unos 1,9 MB, y el `self.cpu_us` de esas
