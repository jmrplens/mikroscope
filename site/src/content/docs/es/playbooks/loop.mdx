...
description: Un bucle de capa 2 en la red de producción del router de referencia que ni el log de RouterOS ni su monitor de puertos mostraron nunca, encontrado por la fuente de log del kernel del agente, diagnosticado a partir de una tasa y desaparecido ese mismo día tras una actualización de firmware, confirmado de dos formas independientes. Volvió una semana después.
---
...
Desde la 1.2.0, `mikroscope doctor` independiente hace este recuento por ti: lee
...
tiene ya el hallazgo de dirección propia. Dos reglas de alerta vigilan el mismo
fallo a lo largo del tiempo: `mikroscope-l2-loop` salta con el primer registro de
