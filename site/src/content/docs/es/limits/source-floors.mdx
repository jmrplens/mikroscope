...
description: Qué fuentes del kernel lee el agente en cada tick y cuáles menos a menudo, el motivo con nombre de cada una y FLOOR_HZ, que desactiva todos los suelos.
head:
  - tag: title
    content: "Cada cuánto se lee cada fuente del kernel RouterOS | mikroscope"
---
...
- **El log del kernel ([`/dev/kmsg`](https://docs.kernel.org/admin-guide/abi-testing.html#symbols-under-dev-kmsg))** se vacía en cada tick y nunca se ralentiza. El valor de un
...
**Temperatura.** En el RB5009 de referencia ambas zonas declaran un [`polling_delay`](https://docs.kernel.org/driver-api/thermal/sysfs-api.html) de 1 000 ms
...
recoge cada movimiento. El [gobernador de cpufreq](https://docs.kernel.org/admin-guide/pm/cpufreq.html#policy-interface-in-sysfs) del RB5009 de referencia marcaba `userspace` el 2026-09-14,
...
Los suelos son constantes con nombre en <Src path="internal/agent/source.go" /> y no números enterrados en la
lógica.
...
```sh
mikroscope install --rate 10 --floor-hz 10   # escribe FLOOR_HZ=10 en la envlist del agente
```
