...
description: Por qué una lectura de CPU no puede ser más fina que el tick de 10 ms del kernel, la única fuente que lee por debajo de él y hasta dónde puede recordar el agente.
---
...
Un tick dura <Measured id="tick.ms" />. `/proc/stat` no cuenta tiempo, cuenta ticks de `USER_HZ`,
...
`/proc/pressure/cpu` de ese kernel lleva línea `full`, cosa que la mayoría de
los kernels no hace.
