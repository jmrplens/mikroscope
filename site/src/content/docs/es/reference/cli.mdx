...
| `--goarm`        | `7`                                        | ninguna                   |                                                              | nivel de `GOARM`, usado solo con `--arch arm`                                                                                                                                                                                                                                                                                                                             |
...
| `--remote-image` | vacío (subir un tar)                       | `MIKROSCOPE_REMOTE_IMAGE` | una referencia de registro, `ghcr.io/owner/name:tag`         | `plan`, `install`, `upgrade`: el router se descarga la imagen él mismo, así que no se compila ni se sube nada, y no queda ningún tar en el equipo. RouterOS toma el host del registro del ajuste global `/container/config registry-url`, que mikroscope nunca escribe; `doctor` lo comprueba y nombra la orden que hay que ejecutar                                      |
