...
`oldest_seq` para leer el anillo entero. Ningún código de la CLI lee
...
la que `doctor` lee el anillo entero (`since` uno por debajo de `oldest_seq`,
`max=10000`, solo por el transporte directo y con el token cuando hay
`TOKEN`). Añade
