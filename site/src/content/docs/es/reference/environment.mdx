...
`install`. Esta página enumera ambas, a partir de los ficheros Go
de <Src path="cmd/mikroscope/" /> y de <Src path="internal/agent/config.go" />.
...
- Entrecomilla una URL que contenga `&` cuando vive en un fichero que cargas
  con `source`: sin comillas, `&` es un operador de la shell. <Src path=".env.example" />
...
| Variable                  | Opción           | La leen                                               | Por defecto en el código        | Valor en <Src path=".env.example" /> |
| ------------------------- | ---------------- | ----------------------------------------------------- | ------------------------------- | ----------------------- |
...
| ------------------------- | -------------- | --------------------------- | ----------------------------------------------------------------------------------------------------- |
| `MIKROSCOPE_API_ADDR`     | `--api`        | `record`, `mark`, `forward` | `host:port` de la API binaria de RouterOS; <Src path=".env.example" /> tiene `192.168.88.1:8728`                   |
...
| `MIKROSCOPE_INTERFACES`   | `--interfaces` | `forward`                   | interfaces separadas por comas para `monitor-traffic`; <Src path=".env.example" /> tiene `bridge,ether1` comentada |
...
| `MIKROSCOPE_HOST_TAG`      | `--host-tag`    | `router`                 | etiqueta de host en cada punto de cada destino; <Src path=".env.example" /> tiene `rb5009` comentada |
...
| `GRAFANA_TOKEN`             | `dashboards import`, `dashboards check` y `forward --grafana` |
...
`MIKROSCOPE_`. <Src path=".env.example" /> lleva las dos, comentadas, en su bloque de Grafana.

`forward` tiene su propio `--grafana`, que publica al arrancar, y sus variables
**sí** llevan el prefijo, porque son ajustes del colector y no del verbo
`dashboards`:

<ScrollTable label="Variables MIKROSCOPE_* de forward --grafana">

| Variable                            | Opción                     | Valor por defecto | Significado                                                    |
| ----------------------------------- | -------------------------- | ----------------- | -------------------------------------------------------------- |
| `MIKROSCOPE_GRAFANA_URL`            | `--grafana`                | vacío             | publicar en este Grafana al arrancar; vacío no publica nada    |
| `MIKROSCOPE_GRAFANA_FOLDER`         | `--grafana-folder`         | `mikroscope`      | carpeta donde publicar; vacío es la carpeta General de Grafana |
| `MIKROSCOPE_GRAFANA_DATASOURCE_UID` | `--grafana-datasource-uid` | vacío             | adoptar este datasource en lugar de crear uno                  |
| `MIKROSCOPE_GRAFANA_DATASOURCE_URL` | `--grafana-datasource-url` | vacío             | la dirección que consulta Grafana, para los destinos que no pueden saberla |
| `MIKROSCOPE_GRAFANA_DATASOURCE_SSLMODE` | `--grafana-datasource-sslmode` | vacío         | sslmode para la fuente de datos de PostgreSQL                  |

</ScrollTable>

El token es `GRAFANA_TOKEN`, el mismo, sin prefijo. `forward --grafana` se niega
a publicar sin él: hay Grafanas que aceptan una petición anónima, y uno que lo
hiciera escribiría como aquel por quien el servidor tome a quien pregunta.
...
de Go que sirve la línea medida en el RB5009 con todas las fuentes activas
(<Measured id="ring.lineBytes" />, 4 núcleos, `IRQ_TOP_K` 8, RouterOS 7.24.2, 2026-09-17), porque es
esa clase lo que se le carga al heap. Los 2 560 B usados antes de la 1.0.5
salían de una línea de 2 439 B medida el 2026-09-12, antes de que llevara la PMU
y los tiempos del propio muestreador, y se quedaban un 35 % cortos con el
anillo. Una placa con más núcleos
...
<ScrollTable label="Límite de memoria frente a RSS y CPU por muestra, RB5009, 2026-09-17" nowrap sticky={false}>

| límite        | múltiplo del anillo | RSS       | CPU por muestra | Lo que costó |
| ------------- | ------------------- | --------- | --------------- | ------------------------- |
| 40 MiB        | 4,0×                | 32,9 MiB  | 2 657 µs        | el límite nunca aprieta   |
| 24 MiB        | 2,4×                | 26,3 MiB  | 2 780 µs        | sin coste medible         |
| 21 MiB        | 2,1×                | 23,5 MiB  | 3 250 µs        | +22 %, y subiendo         |
| 18 MiB        | 1,8×                | 20,4 MiB  | 14 800 µs       | +457 %, peor tick 52 ms   |

</ScrollTable>

Las filas son las que registra el comentario de `memLimitRingFactor` en el
fichero <Src path="internal/router/options.go" />. El factor 2,5× que deriva `install` daría 25 MiB a 300 s; la ventana
medida más cercana fue la de 24 MiB, y ninguna de las cuatro ventanas se anotó con su dispersión.
