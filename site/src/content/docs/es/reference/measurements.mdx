...
description: Cada medida de InfluxDB y cada tabla SQL que escribe mikroscope, con etiquetas, campos, columnas, claves, unidades y si cada valor es un delta o un nivel.
---
...
Está leída de <Src path="internal/sinks/influx.go" />, <Src path="internal/sinks/device.go" /> y del
fichero <Src path="internal/sinks/sql.go" />. Los dos almacenes no guardan el mismo conjunto; [dónde
...
- `forward --stdout lp` y `forward --telegraf` renderizan con el mismo
  ...
  ellas, y el fichero <Src path="internal/sinks/telegraf.go" /> registra 12 renderizados de un mapa de
...
En [line protocol](https://docs.influxdata.com/influxdb3/core/reference/line-protocol/), una `u`
final es un entero sin signo, `i` uno con signo, un número a secas un float,
`true`/`false` un booleano, y un valor entre comillas una cadena.
...
`mikroscope_device_cpufreq` (<Src path="internal/dashboards/panels_p5.go" />); ninguna
