...
description: Cada familia de métricas Prometheus que expone el colector de mikroscope para un MikroTik RouterOS, por fuente, con su tipo, sus labels y cuándo falta.
head:
  - tag: title
    content: Familias de métricas Prometheus de MikroTik RouterOS | mikroscope
---
...
[formato de texto de
Prometheus](https://prometheus.io/docs/instrumenting/exposition_formats/): su
nombre, su tipo, sus labels, qué cuenta y cuándo no está. Está leída
de <Src path="internal/expo/expo.go" />, <Src path="internal/agent/capture.go" />
y <Src path="internal/sinks/prometheus.go" />. Cada familia aparece bajo la fuente de la
que sale, porque eso es lo que decide si una placa concreta la tiene.
...
cadencia del agente (`promHistogramRateHz` en <Src path="cmd/mikroscope/sinkflags.go" />),
...
Las tres primeras se comprobaron contra un agente real el 2026-09-21,
ejecutado a 100 Hz y otra vez a 50 Hz con el colector tirando de él: `/healthz`
daba `rate_hz` 100 y 50 y el propio registro del colector repetía esa misma
cadencia, mientras que `mikroscope_info{rate_hz}` valía `10` en los dos casos;
`mikroscope_cpu_busy_ticks` llevaba los cubos `le="0"` a `le="11"` más `+Inf` en
los dos; y `window="1s"`, `window="10s"` y `window="60s"` estaban los tres
presentes en los dos.

<NotClaimed variant="untested">
	Las dos últimas siguen siendo aritmética, de <Src path="internal/expo/expo.go" />: el `Totals` del colector se
	dimensiona una sola vez con los 10 Hz nominales y `SetSamplerRate` solo redimensiona el anillo,
	nunca rehace los cubos, así que el peso de 1/600 y el desalojo a las 36 000 muestras valen sea
	cual sea la cadencia del agente. No se ha visto ocurrir ninguna de las dos: el desalojo necesita
	que una línea de interrupción se caiga del top-K y siga fuera, y ninguna ejecución lo ha
	provocado.
</NotClaimed>
...
Los tres histogramas los representa el colector a partir del `dt_ns` y del
bloque `self` de cada muestra; los dos contadores salen del `/sampler` del
agente. Una muestra se lee a lo largo de un tramo de tiempo, no en un instante,
y estas familias miden ese tramo.
...
| `mikroscope_sampler_ticks_total` | counter | ninguna | ticks que el agente ha dado desde que arrancó, según los cuenta él |
...
<ScrollTable label="Familias de la PMU y de la frecuencia de CPU">
...
| `mikroscope_perf_time_running_seconds_total` | counter | `counter`, `cpu` | lo mismo                            | segundos que estuvo contando de verdad; por debajo de `enabled` la PMU está multiplexada y los recuentos se escalan a la baja por running sobre enabled                                 |
...
tampoco aquí; en el RB5009 de referencia (RouterOS 7.24.2, kernel 5.6.3) el
2026-09-12, `nbd0`–`nbd15` estaban presentes e inactivos y `mtdblock0`–`2`
daban todo ceros (<Src path="internal/dashboards/panels.go" />, el panel de E/S en curso de
disco). A diferencia de las fuentes con suelo de las convenciones de arriba,
...
desde el principio, a 0, en la exposición cuando las capacidades listan
...
El agente nombra el puerto en sus registros con el nombre por defecto de
RouterOS en una placa de la tabla de puertos, y si no con el nombre del kernel.
El inventario de interfaces de la capa de la API del colector pone en `port` el
nombre actual del puerto en RouterOS, así que un puerto renombrado de `ether5` a
`WAN` se cuenta bajo `WAN`; sin capa de la API, el registro conserva el nombre
por defecto de la placa. El colector clasifica todo registro de puerto que le
llega sin `kind` propio, así que los registros de un agente que no lo envía
también llevan la label.
...
| `mikroscope_derived_cycles_per_packet`       | gauge   | ninguna                              | ciclos de la PMU por paquete, sumados sobre los núcleos; falta sin PMU, en una muestra sin paquetes y en una con un reinicio de contador                                                                                                                 |
...
- [Los endpoints HTTP del agente](/mikroscope/es/reference/http/): `/healthz`, `/capabilities`,
  `/snapshot`, `/stream`, `/sampler` y las rutas de capturas.
