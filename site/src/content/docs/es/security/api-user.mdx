...
head:
  - tag: title
    content: "Usuario de la API de RouterOS con mínimo privilegio | mikroscope"
---
...
- `forward` funciona solo con la capa del kernel. Con una dirección que no responde
  registra `api tier: not connected yet, will keep trying: …` y sigue reintentando.
...
Un grupo que concede `read`, `api` y `test` y niega por su nombre todas las demás políticas
[que define RouterOS](https://help.mikrotik.com/docs/spaces/ROS/pages/8978504/User), y un usuario en él restringido a la dirección del colector:
...
El <Src path=".env.example" /> del repositorio da la dirección como `192.168.88.1:8728`.
