...
| `--remote-image`                        | una referencia de registro: `ghcr.io/owner/name:1.0.0`, sin comillas, espacios ni punto y coma  |
...
- **`--remote-image <referencia>`** hace que el router se descargue la imagen él mismo, así que no
  ...
  **mikroscope nunca escribe ese ajuste.** `doctor` lo lee y, cuando no coincide con la referencia,
  imprime la única orden que hay que ejecutar
  (`/container/config/set registry-url=https://ghcr.io`) o dice que uses `--agent-tar` en su lugar.
  Confiar en la imagen es confiar en ese registro: nada en la CLI verifica lo que el router se
  descarga.
