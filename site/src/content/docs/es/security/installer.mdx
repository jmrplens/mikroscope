...
solo desde 1.0.10, que es cuando `upgrade` empezó a respetar el flag. El listado enmascara el token como
`value="(token)"`.
...
la misma pregunta `write the objects above to the router? [y/N]` —y desde 1.0.10 sí imprime un
...
- **`--remote-image <referencia>`** hace que el router se descargue la imagen él mismo, así que no
  ...
  `jmrplens/mikroscope-agent:1.0.9` no lleva host y deja el ajuste como lo tenga el router. Confiar
