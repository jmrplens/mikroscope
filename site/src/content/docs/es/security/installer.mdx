...
- **`--remote-image <referencia>`** hace que el router se descargue la imagen él mismo, así que no
  ...
  `jmrplens/mikroscope-agent:<versión>` (por ejemplo `:1.2.0`), no lleva host y deja el ajuste como
  lo tenga el router. Confiar en la imagen es confiar en ese registro: nada en la CLI verifica lo
