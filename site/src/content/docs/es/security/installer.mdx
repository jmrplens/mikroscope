...
head:
  - tag: title
    content: "Lo que el instalador rechaza en un router MikroTik | mikroscope"
---
...
1. ejecuta `doctor`, la comprobación previa de solo lectura, salvo que se dé `--no-doctor`; un
   requisito que falte lo detiene con `N prerequisite(s) missing; nothing was written`. Una línea
   `WARN` (una credencial de registro pensada para otro registro, o una instalación ya publicada en
   la LAN sin token) se imprime y no lo detiene;
...
`plan`, `install --dry-run` y `upgrade --dry-run` se detienen después del listado —el último
solo desde 1.1.0, que es cuando `upgrade` empezó a respetar el flag. El listado enmascara el token como
`value="(token)"`.
...
la misma pregunta `write the objects above to the router? [y/N]` —y desde 1.1.0 sí imprime un
listado propio, solo el paso del contenedor. Sigue sin ejecutar `doctor` antes; con `--remote-image` lee `registry-url` y si
hay un usuario de registro puesto, en la misma conexión en la que comprueba que la instalación está, e imprime la comprobación de
la credencial de doctor antes de la pregunta. Con `y` quita el contenedor, la envlist y
...
Todo `find` pone entre comillas cada valor no numérico salvo los enumerados `chain` y `action`. Sin
comillas, una dirección o un puerto se interpretan como valores tipados y la comparación con el
valor guardado sale vacía; verificado para los dos en <Verified of="find-quoting" />. Una palabra
suelta como `tcp` se lee como el nombre de una variable, cuyo valor sin definir es vacío: en el
mismo RB5009 con RouterOS 7.24.4, el 2026-09-21, `find chain=dstnat protocol=tcp` devolvió 0 de 15
reglas dstnat y `protocol="tcp"` devolvió 10. Antes de 1.1.0 el `protocol` de las reglas de expose era
el único valor sin comillas, así que `uninstall --expose` no quitaba ninguna de las dos reglas y las
daba por quitadas.
...
- **`--remote-image <referencia>`** hace que el router se descargue la imagen él mismo, así que no
  ...
  una cadena entrecomillada en una línea unida por `;`. Va entera en `remote-image=`, con el host
  del registro incluido: una referencia sin host, o con un alias de Docker Hub, pasa a ser
  `registry-1.docker.io/…`, y cualquier otro host se conserva. Después el router necesita alcanzar
  ese registro por su propia red. **mikroscope nunca escribe `/container/config`**, el ajuste global
  que guarda el `registry-url` del equipo y su único usuario y contraseña de registro, y ya no
  necesita nada de él: en el router de referencia el host dentro de `remote-image=` mandó sobre
  `registry-url` (verificado en <Verified of="remote-image-host" />), y
  [Nada que ajustar en el router](/mikroscope/es/install/routes/#nada-que-ajustar-en-el-router)
  dice lo que no se ha medido. Confiar en la imagen es confiar en ese registro: nada en la CLI
  verifica lo que el router se descarga. Cuando está puesto, RouterOS presentó ese usuario al
  registro que nombraba `registry-url`: una cuenta de Docker Hub enviada a GHCR terminó una descarga
  en `auth error` el 2026-09-21. No se ha medido si lo presenta a un host que solo nombra
  `remote-image=`. `doctor` avisa cuando hay un usuario puesto y `registry-url` está vacío o nombra
  un host distinto del que viene la imagen (`WARN no registry credential meant for another
  registry`), y `upgrade` imprime la misma comprobación antes de retirar nada; los dos leen
  `registry-url` y solo si hay un usuario puesto, nunca el nombre. La contraseña no se puede leer en
  absoluto. `--agent-tar` no descarga de ningún registro, así que no se envía ninguna
...
`plan`, `install --dry-run` y `upgrade --dry-run` enmascaran el token en lo que imprimen por el
terminal (`value="(token)"`); `--rsc` no puede, porque el script tiene que ejecutarse.
...
La cuenta cubre solo los pasos del plan construido con las propias opciones de `uninstall`, no todo
lo que lleva la etiqueta en el router. Con otras opciones que las de la instalación — sin
`--expose`, u otro `--name`, `--veth`, `--subnet`, `--port`, `--iface-list` o `--addr-list` — pasa
por alto objetos que siguen ahí: en el RB5009 de referencia (RouterOS 7.24.4, 2026-09-21) un
`uninstall` sin `--expose` imprimió `verified: nothing mikroscope created remains on the router`
con el dst-nat de expose todavía en su sitio. Pásale las opciones que tuvo `install`; [órdenes y
opciones](/mikroscope/es/reference/cli/#pasa-la-misma-forma-a-status-upgrade-y-uninstall) tiene la
prueba.
...
<NotClaimed variant="device-specific">
	...
	errores impresos con estado de salida 0 — se observaron en un RB5009 con RouterOS 7.24.2, salvo el
	caso de `protocol=tcp` sin comillas, medido en el mismo equipo con 7.24.4. La lógica de rechazo
	en sí está cubierta por pruebas contra un router simulado, no por una ejecución en otra placa u
	otra versión de RouterOS.
</NotClaimed>
