...
listado propio, solo el paso del contenedor. Sigue sin ejecutar `doctor` antes. Con `y` quita el contenedor, la envlist y
...
- **`--remote-image <referencia>`** hace que el router se descargue la imagen él mismo, así que no
  ...
  ajuste global del dispositivo y compartido con todos los demás contenedores que haya en él.
  RouterOS lo trae puesto en `https://lscr.io` desde la 7.18, y ahí no está la imagen; en el RB5009
  de referencia vale `https://registry-1.docker.io`. **mikroscope nunca escribe ese ajuste.** `doctor`
  ...
  `jmrplens/mikroscope-agent:<versión>` (por ejemplo `:{{MIKROSCOPE_VERSION}}`), no lleva host y deja el ajuste como
  lo tenga el router, así que un router aún en el valor por defecto de RouterOS necesita antes
  `https://registry-1.docker.io`, como dice
  [Un pull del registro](/mikroscope/es/install/routes/#un-pull-del-registro). Confiar en la imagen es confiar en ese registro: nada en la CLI verifica lo
  que el router se descarga. El mismo ajuste guarda un único usuario y contraseña de registro para
  todo el equipo, y RouterOS los presenta a cualquier registro del que descargue, así que una cuenta
  de Docker Hub se envía a GHCR y la descarga termina en `auth error`. `doctor` avisa de ello
  (`WARN no registry credential meant for another registry`) y solo lee si hay un usuario puesto,
  nunca el nombre. La contraseña no se puede leer en absoluto. `--agent-tar` no descarga de ningún
  registro, así que no se envía ninguna credencial de registro.
