...
Informa de una vulnerabilidad en privado, nunca con sus detalles en un issue público: abre la
[pestaña Security](https://github.com/jmrplens/mikroscope/security) del repositorio y elige **Report
a vulnerability**. GitHub abre entonces un aviso privado que solo ve el mantenedor. Si ese botón no
aparece, abre un issue que pida un canal privado y no diga nada más del problema, y espera ahí una
respuesta que te lo indique. Incluye la versión (`mikroscope version`, y
`mikroscope-agent -version` si es distinta), la versión de RouterOS y la placa, lo que gana un
atacante y la forma más corta de reproducirlo. Si una prueba de concepto necesita una credencial,
una dirección de router o un export, no lo envíes: descríbelo.
