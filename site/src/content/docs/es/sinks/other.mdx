...
description: "El fichero JSONL, stdout, SQL y PostgreSQL, Loki, OTLP, Graphite, Elasticsearch y Telegraf: qué lleva cada destino, cómo entrega y qué no puede prometer."
---
...
<Aside type="caution" title="Una contraseña en un DSN se ve en la lista de procesos">
...
conocido, enviado como JSON al [endpoint de push](https://grafana.com/docs/loki/latest/reference/loki-http-api/)
de Loki, `/loki/api/v1/push`. Un token bearer sale de `MIKROSCOPE_LOKI_TOKEN`, y
`--loki-tenant` (`MIKROSCOPE_LOKI_TENANT`) fija `X-Scope-OrgID`, la cabecera de la que un
[Loki multi-tenant](https://grafana.com/docs/loki/latest/operations/multi-tenancy/) lee el tenant.
...
runtime. La [especificación de OTLP](https://opentelemetry.io/docs/specs/otlp/) dice que
un servidor SHOULD —debería, no está obligado a— aceptar cargas codificadas en JSON en el
mismo puerto que el protobuf binario, así que un receptor que solo acepte protobuf la
cumple al pie de la letra. El único receptor real contra el que se ha ejecutado este
destino, el OpenTelemetry Collector de la batería de extremo a extremo con docker, acepta
JSON. El recurso
...
escrito y se registra, no se reintenta, que es lo que la especificación de OTLP exige a un
cliente. El rechazo es determinista (un receptor OTLP de
...
`--graphite` (`MIKROSCOPE_GRAPHITE_ADDR`) escribe el [protocolo de texto
plano](https://graphite.readthedocs.io/en/latest/feeding-carbon.html) de carbon —
...
dos productos: la de [Elasticsearch](https://www.elastic.co/docs/api/doc/elasticsearch/operation/operation-bulk) y la de
[OpenSearch](https://docs.opensearch.org/latest/api-reference/document-apis/bulk/) aceptan el mismo JSON delimitado por
saltos de línea. `/_bulk` se añade a la raíz de un clúster, conservando cualquier query
...
- `http://` o `https://` — envía a una entrada `http_listener_v2` o
  ...
  HTTP sin ruta se le da `/telegraf`, la ruta que da la [configuración de ejemplo de
  `http_listener_v2`](https://github.com/influxdata/telegraf/blob/master/plugins/inputs/http_listener_v2/README.md): un
