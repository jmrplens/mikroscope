---
title: El colector
description: Cómo lleva forward la telemetría de kernel de MikroTik RouterOS a once destinos (extraer, fusionar, derivar, repartir) y qué descarta con un almacén lento.
head:
  - tag: title
    content: El colector de telemetría de MikroTik RouterOS | mikroscope
---
...
router MikroTik, muestrea la capa de la API de RouterOS, marca las dos con el reloj
del agente, pasa la etapa de derivación sobre ellas y escribe la línea temporal
fusionada en cada destino que indiques. Esta página responde a qué hace una ejecución, cómo sigue el ritmo del
...
`--influx`, `--loki`, `--otlp`, `--graphite`, `--elastic`, `--sql`, `--postgres`,
`--telegraf` o `--stdout`, y lo normal es usar más de uno a la vez.
...
2. **Entrega a cada destino el flujo de datos del equipo.** Las `/capabilities` del
   agente — placa, kernel, techos, cadencias — salen al arrancar como registro
   propio, de nuevo cuando cambia el hash de capacidades y, si no, cada cinco
   minutos, para que los paneles del equipo tengan una fila dentro de cualquier
   ventana. Consulta [el flujo de datos del equipo](/mikroscope/es/sinks/device-info/).
...
3. **Tira del anillo cada `--poll`.** La primera extracción empieza después de la
   ...
   tenía antes de arrancar: siempre empieza en la muestra más reciente del agente.
   `forward` no tiene `--from-start` y lo rechaza como opción desconocida; solo
   `record` recupera el anillo. Cada extracción pide las
   muestras posteriores al último número de secuencia visto. Un marcador de disparo
   viaja entre las muestras en orden de secuencia y se reenvía como anotación, nunca
   se decodifica como muestra.
...
El transporte por relay limita una extracción a <Measured id="relay.maxBatch" /> líneas, y el límite
se calcula en vez de elegirse: `/tool fetch` devuelve como
mucho <Measured id="relay.replyMaxBytes" />, la línea del anillo se cobra
a <Measured id="ring.lineChargedBytes" /> (los <Measured id="ring.lineBytes" /> medidos, redondeados a
su clase de tamaño del asignador) y el límite deja un 134 % de eso para que también quepa un lote de
líneas por encima de la media: unos 45 kB. Una respuesta que aun así llega al límite de fetch se rechaza
con `relay reply hit the 64512-byte fetch limit; lower the batch` en vez de analizarse truncada.
Con el sondeo por defecto de 500 ms eso son 26 muestras/s, y por encima el colector se queda atrás.
...
Un colector que se queda más atrás que el anillo del agente (60 s por defecto) recibe
...
| quieres escribir directamente en un PostgreSQL en marcha      | `--postgres`   | las sentencias de `--sql`, por una conexión         | **sí**, generado |
| quieres el registro del kernel y las detecciones con tus logs | `--loki`       | **solo eventos**: kmsg, detecciones, huecos, disparos, errores de la API, registros del equipo | no |
...
métricas** —los registros del kernel, las detecciones, los huecos, los disparos,
los errores de la capa de la API y los registros del equipo—, así que una ejecución
solo con Loki no tiene ni un número de CPU o memoria. Y **`--prom`
...
| `--influx URL`, `--influx-db DB` | protocolo de líneas de InfluxDB 3                  | `MIKROSCOPE_INFLUX_URL`, `MIKROSCOPE_INFLUX_DB`, `MIKROSCOPE_INFLUX_TOKEN` | en cola    | [InfluxDB 3](/mikroscope/es/sinks/influxdb/)                            |
...
| `--postgres DSN`                | las mismas sentencias, a un PostgreSQL en marcha    | `MIKROSCOPE_POSTGRES_DSN`                                                | en cola    | [`--postgres`](/mikroscope/es/sinks/other/#--postgres-las-mismas-sentencias-por-una-conexión) |
...
- La cola guarda `--queue-seconds` (60 por defecto) segundos de un presupuesto de bytes:
  64 KiB por segundo para InfluxDB, PostgreSQL (`--postgres`), Loki, OTLP,
  Elasticsearch, Telegraf y stdout, y 256 KiB por segundo para Graphite, cuyo formato
  de una línea por valor ocupa más.
...
- Cada POST HTTP lleva un tiempo de espera de 10 s, y una conexión reutilizada muerta es
  un error que se reintenta y se cuenta, no un reenvío silencioso. `--postgres` envía
  cada lote como una sola transacción con un tiempo de espera de 30 s.
...
<NotClaimed>
	Loki, OTLP, Graphite, Elasticsearch, OpenSearch, Telegraf, SQL y stdout se prueban contra
	receptores locales que comprueban los bytes que acepta cada protocolo (máquina de desarrollo,
	amd64, desde el 2026-09-12). Desde el 2026-09-17 la suite de extremo a extremo con docker además
	ejecuta el colector, contra un agente enlatado y con la capa de la API apagada, hacia Loki 3, el
	OpenTelemetry Collector, graphite-statsd, Elasticsearch 9, Telegraf 1.39 por HTTP y PostgreSQL 18
	(con `--sql`, y con `--postgres` desde que ese destino llegó el 2026-09-21), y lee los datos de
	vuelta por la API de cada almacén. Ninguno se ha alimentado desde el RB5009 hacia un backend en
	marcha, y OpenSearch, las hypertables de TimescaleDB y stdout nunca se han probado contra un
	almacén real. Los tamaños en bytes que se citan para ellos en [los demás
	destinos](/mikroscope/es/sinks/other/) salen de fixtures de pruebas, no de un equipo.
</NotClaimed>
