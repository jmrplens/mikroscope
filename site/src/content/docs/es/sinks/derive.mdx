...
description: "Qué calcula el colector junto a las muestras: presión de memoria, coste PMU por paquete, paquetes por interrupción, la marca de ráfaga y cuándo se omite."
---
...
### Las razones por paquete de la PMU
...
3 738 704 muestras por CPU en 24 h: alrededor del <Measured id="squeeze.backgroundOne" /> de las muestras llevan un squeeze de fondo
y el <Measured id="squeeze.backgroundTwo" /> llevan exactamente dos. «Cualquier squeeze» marcaría la normalidad del equipo varias
veces por minuto.
...
`fp_tx_bytes` siempre. Leído en el código (<Src path="internal/derive/derive.go" />), no observado. En ese
...
| SQL (`--sql`, `--postgres`)     | `mikroscope_derived`, cocientes NULL donde no se calcularon                                        | `mikroscope_derived_iface`                                           |
...
Prometheus escribe `mikroscope_derived_memory_pressure`, `mikroscope_derived_cycles_per_packet`,
`mikroscope_derived_instructions_per_packet`, `mikroscope_derived_cache_misses_per_packet`
y `mikroscope_derived_packets_per_interrupt` de la muestra más reciente; `burst` solo le
llega como el contador `mikroscope_collector_bursts_total`, y `suspect` no le llega. OTLP
escribe `mikroscope.derived.memory_pressure`, `mikroscope.derived.cycles_per_packet`,
`mikroscope.derived.instructions_per_packet`, `mikroscope.derived.cache_misses_per_packet`
y `mikroscope.derived.packets_per_irq`. Graphite escribe `derived.mem_pressure`,
`derived.cycles_per_packet`, `derived.instructions_per_packet`,
`derived.cache_misses_per_packet` y `derived.packets_per_irq`. Ni OTLP ni Graphite llevan
