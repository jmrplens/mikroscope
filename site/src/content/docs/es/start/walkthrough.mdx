...
`VETH`, `SUBNET`, `IFACE_LIST`, `ADDR_LIST`, `DISK`, `ARCH`, `TOKEN`, `LAN_ADDRESS`), las del
origen de la imagen (`AGENT_TAR`, `REMOTE_IMAGE`), las de la API (`API_ADDR`, `API_USER`,
`API_PASSWORD`), las URL de los destinos como `INFLUX_URL`, `INTERFACES`, `HOST_TAG` y los
`GRAFANA_*` del paso 7. `GRAFANA_TOKEN` se lee con ese nombre exacto, sin prefijo. El resto, entre
ellas `--ephemeral`, `--rate`, `--for`, `--topics` y `--prom`, tiene valores por defecto fijos,
diga lo que diga el texto de uso de la CLI. Exporta las variables que
...
1. **Conseguir mikroscope.**
   ...
   ```sh wrap
   tar xzf mikroscope_1.2.0_linux_x86_64.tar.gz   # un .zip en Windows
   ./mikroscope version
   ```
   ...
   como `jmrplens/mikroscope-agent:1.2.0` en Docker Hub como
   `ghcr.io/jmrplens/mikroscope-agent:1.2.0` en GHCR; el paso 2 toma uno de los dos.
...
2. **Instalar, una vez.**
   ...
   - **el registro**, `--remote-image jmrplens/mikroscope-agent:1.2.0`: el router se descarga la
     ...
     `ghcr.io/jmrplens/mikroscope-agent:1.2.0`, nombra un host propio: `doctor` compara el ajuste
   ...
   `mikroscope plan --rsc --remote-image jmrplens/mikroscope-agent:1.2.0 --out install.rsc`
