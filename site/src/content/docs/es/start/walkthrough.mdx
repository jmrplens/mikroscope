...
1. **Conseguir mikroscope.**
   ...
   ```sh wrap
   tar xzf mikroscope_1.0.9_linux_x86_64.tar.gz   # un .zip en Windows
   ./mikroscope version
   ```
   ...
   como `jmrplens/mikroscope-agent:1.0.9` en Docker Hub como
   `ghcr.io/jmrplens/mikroscope-agent:1.0.9` en GHCR; el paso 2 toma uno de los dos.
...
2. **Instalar, una vez.**
   ...
   - **el registro**, `--remote-image jmrplens/mikroscope-agent:1.0.9`: el router se descarga la
     ...
     `ghcr.io/jmrplens/mikroscope-agent:1.0.9`, nombra un host propio: `doctor` compara el ajuste
   ...
   `mikroscope plan --rsc --remote-image jmrplens/mikroscope-agent:1.0.9 --out install.rsc`
