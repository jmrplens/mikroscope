...
1. **Conseguir mikroscope.**
   ...
   `mikroscope-agent-arm.tar`, `mikroscope-agent-amd64.tar`) y como
   `ghcr.io/jmrplens/mikroscope-agent:1.0.0`; el paso 2 toma uno de los dos.
...
2. **Instalar, una vez.**
   ...
   - **el registro**, `--remote-image ghcr.io/jmrplens/mikroscope-agent:1.0.0`: el router se
     descarga la imagen él mismo, no se sube nada y `uninstall` no tiene ningún fichero del que
     responder. RouterOS toma el host del registro del ajuste global
     `/container/config registry-url`, que mikroscope nunca escribe porque lo comparten todos los
     contenedores del equipo; `doctor` lo comprueba e imprime
     `/container/config/set registry-url=https://ghcr.io` cuando no coincide, o remite a
     `--agent-tar`. La descarga necesita que el router llegue al registro y que haya RAM libre para
     las capas.
   ...
   `mikroscope plan --rsc --remote-image ghcr.io/jmrplens/mikroscope-agent:1.0.0 --out install.rsc`
