...
description: Una grabación real de un RB5009 leída línea a línea, y los cinco pasos que la producen — la CLI, instalar el agente, grabar, añadir el log, dibujar.
head:
  - tag: title
    content: Cinco minutos con un router MikroTik | mikroscope
---
...
un router que nunca ha visto mikroscope hasta un gráfico que la responde: cinco pasos hasta el
gráfico y dos opcionales para dejarlo en marcha, con las órdenes tal cual son, y una grabación real
del RB5009 leída línea a línea.

## Antes de empezar

El router necesita lo que la herramienta no le puede dar: RouterOS 7.24 o posterior con el
[paquete `container`](https://help.mikrotik.com/docs/spaces/ROS/pages/84901929/Container) y
[`device-mode container=yes`](https://help.mikrotik.com/docs/spaces/ROS/pages/93749258/Device-mode),
que MikroTik protege tras una pulsación del botón de reset o un ciclo de alimentación. El suelo es
7.24 porque el paso del contenedor escribe `privileged=`, un atributo que MikroTik
[añadió en esa versión](https://download.mikrotik.com/routeros/7.24/CHANGELOG);
`--privileged=false` cambia su valor, no si se escribe, así que una 7.x anterior debería rechazar
la orden igualmente (sale del código; no se ha probado en un router). Todo lo de esta
...
Las órdenes de esta página necesitan seis variables de entorno:

<ScrollTable label="Las seis variables de entorno que necesitan las órdenes de esta página">

| Variable                  | Opción       | Se lee en                           |
| ------------------------- | ------------ | ----------------------------------- |
| `MIKROSCOPE_ROUTER`       | `--router`   | el paso 2 y la desinstalación final |
| `MIKROSCOPE_API_ADDR`     | `--api`      | los pasos 4, 6 y 7                  |
| `MIKROSCOPE_API_USER`     | `--api-user` | los pasos 4, 6 y 7                  |
| `MIKROSCOPE_API_PASSWORD` | ninguna      | los pasos 4, 6 y 7                  |
| `MIKROSCOPE_INFLUX_URL`   | `--influx`   | el paso 6                           |
| `GRAFANA_TOKEN`           | ninguna      | el paso 7 y la desinstalación final |

</ScrollTable>

Una variable con opción solo fija el valor por defecto de esa opción, y la opción en la línea de
órdenes manda; las dos que no tienen ninguna son credenciales, que la CLI lee solo del entorno para
que no aparezcan en `ps` ni en el historial de tu shell. `GRAFANA_TOKEN` se lee con ese nombre
exacto, sin prefijo. La CLI nunca lee `.env` por sí misma: exporta las variables, o copia el
fichero <Src path=".env.example" /> a `.env` y cárgalo con `set -a; . ./.env; set +a`. Las mismas
órdenes leen también otras variables `MIKROSCOPE_*` cuando están definidas, entre ellas
`MIKROSCOPE_TOKEN` y `MIKROSCOPE_SUBNET`. La mayoría
de las opciones no tiene variable —`--ephemeral`, `--rate`, `--for`, `--topics` y `--prom`, entre
otras—, y [variables de entorno](/mikroscope/es/reference/environment/) lista todas las que sí.
...
1. **Conseguir mikroscope.**
   ...
   Eso averigua la plataforma, toma la versión más reciente y se niega a instalar nada cuya suma
   de comprobación no sea la que publicó la release; `irm …/install.ps1 | iex` es la mitad de
   PowerShell. [Conseguir la CLI](/mikroscope/es/install/cli/) tiene el mismo camino a mano,
   plataforma por plataforma, para una máquina donde `curl | bash` no es algo que se teclee:

   ```sh wrap
   tar xzf mikroscope_{{MIKROSCOPE_VERSION}}_linux_x86_64.tar.gz   # un .zip en Windows
   ./mikroscope version
   ```

   La versión publicada trae la CLI como un archivo por plataforma —linux y FreeBSD en amd64,
   arm64 y arm; macOS y Windows en amd64 y arm64— con `checksums.txt`, las firmas de cosign y los
   SBOM al lado. El
   agente viaja aparte, como cuatro tars de imagen, uno por arquitectura
   (`mikroscope-agent-amd64.tar`, `-arm64`, `-armv5`, `-armv7` —el ARM de 32 bits se partió en dos
   en 1.0.2, y el tar equivocado da `exec format error`) y como imagen de registro, publicada tanto
   como `jmrplens/mikroscope-agent:{{MIKROSCOPE_VERSION}}` en Docker Hub como
   `ghcr.io/jmrplens/mikroscope-agent:{{MIKROSCOPE_VERSION}}` en GHCR; el paso 2 toma uno de los dos.
...
2. **Instalar, una vez.**
   ...
   Desde la 1.2.0 un `doctor` a solas tiene además una segunda mitad, tras la comprobación previa:
   `health (what the running agent's ring shows now):`. En un router que aún no tiene agente
   espera hasta 3 s e imprime `skipped: no agent answered at …`, lo esperable antes de la primera
   instalación. Si lo vuelves a ejecutar con el agente en marcha, lee su anillo (los últimos 60 s
   con el `--buffer` por defecto) e informa de una firma de bucle de capa 2, churn de STP, un link
   flap o descartes de softnet. Lo que encuentre nunca cambia el código de salida de `doctor`, así
   que el `&&` de arriba sigue llegando a `install`.
   ...
   - **el registro**, `--remote-image jmrplens/mikroscope-agent:{{MIKROSCOPE_VERSION}}`: el router se descarga la
     imagen él mismo, no se sube nada y `uninstall` no tiene ningún fichero del que responder.
     mikroscope le da a RouterOS la referencia entera,
     `registry-1.docker.io/jmrplens/mikroscope-agent:{{MIKROSCOPE_VERSION}}`, así que el ajuste global
     `/container/config registry-url`, que comparten todos los contenedores del equipo, no decide
     la descarga: no hay que poner nada ahí antes y mikroscope nunca lo escribe. Tampoco hace falta ningún
     login de registro. En el router de referencia el host dentro de `remote-image=` mandó sobre
     `registry-url` (verificado en <Verified of="remote-image-host" />), y Docker Hub sirvió el
     agente con el usuario y la contraseña de registro del equipo borrados (verificado
     en <Verified of="docker-hub-anonymous" />); no se ha probado un router con `registry-url` en el
     valor de fábrica. La referencia de GHCR,
     `ghcr.io/jmrplens/mikroscope-agent:{{MIKROSCOPE_VERSION}}`, sale igual con su propio host. Desde
     la 1.2.0 `doctor` imprime `WARN` cuando `/container/config` tiene un usuario de registro que
     puede no pertenecer al registro de la descarga; después de la 1.2.2 lo decide comparando el
     host que nombra `registry-url` con el del que viene la imagen. Esa credencial es una para todo
     el equipo, y una cuenta de Docker Hub enviada a GHCR terminó una descarga en `auth error`. Un
     `WARN` no detiene `install`. La descarga necesita que el router llegue al registro y que haya
     RAM libre para las capas.
   ...
   `mikroscope plan --rsc --remote-image jmrplens/mikroscope-agent:{{MIKROSCOPE_VERSION}} --out install.rsc`
   ...
   `install` termina sondeando al agente desde tu equipo; la última de sus líneas es la del sondeo:
   ...
   desde el fichero <Src path="VERSION" />, y después el commit y la fecha de compilación. Un `go build` sin
...
6. **Dejarlo en marcha (opcional).**
   ...
   InfluxDB 3. Basta con un trabajo de scrape: el del colector. Las reglas de alerta para
   InfluxDB 3, PostgreSQL y Prometheus van junto a los paneles y se importan aparte;
   [alertas](/mikroscope/es/dashboards/alerts/) las lista.

7. **Deja que monte Grafana por ti (opcional).**
   ...
   Desde la 1.1.0 el colector puede hacer él mismo la última frase del paso 6: apúntalo a un
   Grafana y crea la fuente de datos y publica el dashboard, una vez, al arrancar, antes de la
   primera muestra. No hace nada si no se pasa `--grafana`, un Grafana que no lo acepte es un
   aviso y no un motivo para no recoger, y no borra nunca nada. `--grafana-dry-run` imprime lo
   que escribiría y para antes de tocar el router, así que puedes leerlo primero.
   [Importar y comprobar](/mikroscope/es/dashboards/import-and-check/) dice qué destinos saben
   describir su propia fuente de datos y a cuáles hay que decirles la dirección.
...
```sh wrap
bin/mikroscope uninstall --ephemeral          # lista lo que quitaría
bin/mikroscope uninstall --ephemeral --yes    # y lo quita
```
...
Sin `--yes` solo lista lo que quitaría. `bin/mikroscope status --ephemeral` imprime esos mismos recuentos de
propiedad en cualquier momento, más la línea de `/healthz` del agente (versión, cadencia,
secuencia, tiempo en marcha, ticks perdidos) cuando se llega a él; el verbo que lee su anillo en
busca de fallos es `doctor`.

Si hiciste el paso 7, `uninstall --targets dashboard --grafana <url>` con `GRAFANA_TOKEN` y las
mismas opciones de destino que `forward` quita el dashboard y la fuente de datos que creó, nunca
una adoptada con `--grafana-datasource-uid`. `--targets data` con esas opciones vacía lo que el
paso 6 escribió en cada almacén que puede vaciar, y nombra los que no. Nada se va sin `--yes`;
[`uninstall --targets`](/mikroscope/es/reference/cli/#uninstall---targets) tiene la tabla.
