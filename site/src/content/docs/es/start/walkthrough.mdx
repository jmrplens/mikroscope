...
2. **Instalar, una vez.**
   ...
   - **el registro**, `--remote-image jmrplens/mikroscope-agent:{{MIKROSCOPE_VERSION}}`: el router se descarga la
     ...
     mikroscope nunca escribe porque lo comparten todos los contenedores del equipo. En el RB5009
     vale `https://registry-1.docker.io`, y la referencia de Docker Hub de arriba se descargó de ahí
     tal cual. RouterOS, en cambio, lo trae puesto en `https://lscr.io` desde la
     [7.18](https://download.mikrotik.com/routeros/7.18/CHANGELOG), que no es ninguno de los dos
     registros en los que se publica la imagen, y `doctor` no lo comprueba para una referencia sin
     host: en un router que nadie haya apuntado a Docker Hub, ponlo antes en
     `https://registry-1.docker.io`, o usa el tar. La referencia de GHCR,
     `ghcr.io/jmrplens/mikroscope-agent:{{MIKROSCOPE_VERSION}}`, nombra un host propio: `doctor` compara el ajuste
     con él e imprime `/container/config/set registry-url=https://ghcr.io` cuando no coincide, o
     remite a `--agent-tar`. Desde la 1.2.0 imprime además `WARN` cuando `/container/config` tiene
     un usuario de registro y la descarga va a otro sitio que no es Docker Hub: RouterOS presenta
     esa única credencial a todos los registros, y una cuenta de Docker Hub enviada a GHCR termina
     en `auth error`. Un `WARN` no detiene `install`. La descarga necesita que el router llegue al
     registro y que haya RAM libre para las capas.
