...
un contenedor del router (la imagen de 1.0.0 medía <Measured id="image.size" />; el tamaño de la
imagen actual está por medir sobre la imagen publicada de 1.2.1); `uninstall` la vuelve a quitar. En <Verified of="export-identical" />, un ciclo
...
solo quita con `--yes`. `scripts/roundtrip.sh` pasa ahora `--yes` a su paso de
