...
description: Un ciclo con guion dejó el /export del RB5009 idéntico salvo la cabecera — qué escribe `mikroscope install` en RouterOS, en qué orden, y cómo se quita.
head:
  - tag: title
    content: Instalar el agente en un router MikroTik | mikroscope
---
...
router y cómo recuperas el router después. `install` pone la imagen del agente en
un contenedor del router (la
imagen publicada de 1.2.1 para arm64 mide <Measured id="image.size" />, casi todo el binario del agente); `uninstall` la vuelve a quitar. En <Verified of="export-identical" />, un ciclo
...
de cabecera `#`. Esa ejecución es anterior a 1.1.0, desde la que `uninstall`
solo quita con `--yes`, y <Src path="scripts/roundtrip.sh" /> pasa ahora `--yes` a su paso de
desinstalación (hasta 1.2.0 no lo hacía, así que ahí solo enumeraba y su
comprobación del export fallaba); el guion en esa forma aún no se ha ejecutado
contra el router. Cada objeto que crea install lleva el comentario
...
— esto último exige pulsar un botón físico o un ciclo de alimentación.
...
imprime la orden exacta o el paso físico para lo que falte; después, por HTTP,
lee el anillo de un agente que ya esté en marcha.
...
3. **Ejecuta `doctor`.** Cualquier requisito que falte detiene la instalación
   con `N prerequisite(s) missing; nothing was written`. Una línea `WARN` se
   imprime y no la detiene. La sección de salud del agente del `doctor` por sí
   solo no se ejecuta aquí. `--no-doctor` se salta este paso.
...
6. **Sube la imagen y crea el contenedor.** El tar sube con `scp`; después una
   ...
   `remote-image=` y la referencia entera, con el host del registro incluido, y
   se arranca; `/container/config` no se toca. En cualquier caso el contenedor se añade con
   `privileged=`, que RouterOS conoce desde 7.24: en una 7.x anterior este es el
   paso que debería fallar, diga lo que diga `--privileged` (sale del código, no
   se ha probado en un router), y la salida es actualizar RouterOS.
...
crear el paso con la imagen nueva y sondea el agente.

Antes imprime su propio plan —solo el paso del contenedor, con la línea
`keeps: the veth, the router address and the list memberships are not touched`,
porque esos cuatro objetos son justo los que una actualización NO escribe— y
`--dry-run` se detiene ahí, antes incluso de abrir una conexión con el router.
A diferencia de `install` sigue sin ejecutar `doctor`; ejecútalo tú si quieres
las comprobaciones de espacio libre y device-mode antes de actualizar. Con
`--remote-image` sí imprime la comprobación de la credencial de registro de
doctor, y una `note` cuando `registry-url` nombra un host distinto del de la
descarga, desde la misma conexión en la que comprueba que la instalación está
([Nada que ajustar en el router](/mikroscope/es/install/routes/#nada-que-ajustar-en-el-router)).

Hasta 1.1.0 `upgrade` ignoraba `--dry-run` por completo: no imprimía nada e iba
directo a la confirmación, así que `upgrade --dry-run --yes` reemplazaba el
contenedor en un router vivo mientras el flag prometía no escribir nada.
...
las dos reglas del cortafuegos hacia la LAN se quedan. `doctor` detecta ese
estado después: su línea `WARN`
`the installed agent published on the LAN asks for a token` muestra
...
`mikroscope uninstall` a secas solo enumera: imprime
`router objects (add --yes to remove them):` y los objetos que describen sus
opciones, del más reciente al más antiguo, sin conectarse al router, y no quita
nada. Con `--yes` ejecuta las eliminaciones de la más reciente a la más
...
`--targets` elige qué se quita: `router` (por defecto), `dashboard`, `data` o
`all`; lo detalla [Órdenes y opciones](/mikroscope/es/reference/cli/).
...
```sh
mikroscope doctor                      # solo lectura: requisitos y después el anillo del agente en marcha
...
mikroscope uninstall --yes             # quita y verifica (sin --yes: solo enumera)
...
(`MIKROSCOPE_ROUTER`, `MIKROSCOPE_SSH_PORT` y así sucesivamente); el fichero <Src path=".env.example" />
las documenta. La CLI no lee `.env` por sí misma:
...
Esa lectura es anterior al agente y se tomó sobre RouterOS 7.24.1; sobre 7.24.2, el 2026-09-11, el
mismo coste apareció en `/tool profile` como un 17–33 % en una o dos instantáneas, no en una ventana
medida.
...
<NotClaimed variant="untested">
	...
	sea el RB5009, en cualquier RouterOS que no sea 7.24.2 —install, status y uninstall se han
	ejecutado desde entonces sobre 7.24.4, pero el ciclo en sí no se ha repetido allí—, o con install
	y upgrade ejecutados sin `--ephemeral` (la ejecución con guion lo pasó a doctor, install y
	upgrade, no a status ni a uninstall). El ciclo no se ha ejecutado contra los ajustes actuales del
	contenedor: `privileged=yes`, `memory-max=64M` y las entradas de envlist `MEM_LIMIT_MB`,
	`CAPTURE_MB`, `TRIGGERS` y `FLOOR_HZ`; se ejecutó con `memory-max=32M`.
</NotClaimed>
