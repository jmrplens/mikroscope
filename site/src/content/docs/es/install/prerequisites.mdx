...
comprobaciones. Lee el anillo una vez desde esta máquina —la misma dirección que
sondea `status`— y se rinde a los 3 s, diciéndolo, cuando no responde ningún
agente. Nunca cambia el código de salida: eso lo deciden los requisitos.

El anillo es de 60 s por defecto, así que esto ve lo que pasa ahora, no lo que
pasó esta mañana; el histórico lo leen los dashboards y las reglas de alerta. Lo
que busca, en recuentos de sucesos que un router sano no produce y no en
umbrales ajustados a un equipo:
...
- **`layer2-loop`**: tres o más tramas en la ventana que volvieron a entrar por
  ...
  de referencia lo repetía cada 2,0 s, el intervalo de hello de STP, mientras
...
- **`stp-churn`**: un puerto que STP pasó a learning al menos tres veces más
  ...
  cada link-up sano registra tres de golpe y llega a forwarding 2–3 s después;
  en los 30 días de datos del router de referencia, todos los link-ups sanos
  dejaron learning menos forwarding en 0.
