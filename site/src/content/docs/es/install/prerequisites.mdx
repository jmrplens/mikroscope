...
que `install` borra el tar; en `install --remote-image` no se sube ningún tar,
así que ahí la comprobación pide solo los 4 MiB de margen. `doctor` por sí solo
no hace esa excepción: con `--remote-image` sigue suponiendo la imagen de 7 MiB
y pide 18.0 MiB. El umbral de memoria sigue a
...
Pide al agente como mucho 10 000 muestras, empezando por la más antigua que
guarda el anillo, y todo dentro de los mismos 3 s: un anillo más largo —más de
1 000 s a 10 Hz, más de 100 s a 100 Hz— se lee por su extremo más antiguo, no
por el más reciente. Lo que busca, en recuentos de sucesos que un router sano no
...
- **`layer2-loop`**: tres o más tramas en la ventana que volvieron a entrar por
  ...
  de referencia (2026-09-12, y otra vez del 2026-09-19 al 09-23) lo repetía cada
  2,0 s, el intervalo de hello de STP —treinta veces en un anillo de 60 s—, mientras
...
- **`link-flap`**: dos o más caídas de enlace en un puerto, el mismo recuento
  que usa la marca de link-flap del propio colector.
