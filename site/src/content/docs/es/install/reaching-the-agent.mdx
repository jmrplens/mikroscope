...
| relay      | un usuario de la API con `read,api,test`; `--transport relay` o `auto` | <Measured id="relay.replyMaxBytes" /> y 18 muestras por llamada, ~1 s en más o menos la mitad de las llamadas, sin token |
