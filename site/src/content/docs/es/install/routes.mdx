...
mikroscope install --router usuario@192.168.88.1 \
  --remote-image jmrplens/mikroscope-agent:1.0.6
```
...
`/container/add remote-image="jmrplens/mikroscope-agent:1.0.6" …` y el plan
imprime `the router pulls … (nothing is uploaded)` donde iría la línea de
subida. La release publica la imagen dos veces, como
`jmrplens/mikroscope-agent:1.0.6` en Docker Hub y como
`ghcr.io/jmrplens/mikroscope-agent:1.0.6` en GHCR. Las dos llevan `linux/amd64`,
...
Una referencia sin host —`jmrplens/mikroscope-agent:1.0.6`— deja el registro a
...
`jmrplens/mikroscope-agent:1.0.6` desde Docker Hub con 2 ms, y el script de
...
1. Descarga `mikroscope_1.0.6_<so>_<arch>.tar.gz` (`.zip` en Windows) y el tar
...
3. Desempaqueta la CLI e instala:
   ...
   ```sh
   tar xzf mikroscope_1.0.6_linux_x86_64.tar.gz
   ./mikroscope install --router usuario@192.168.88.1 \
...
<Aside type="tip" title="Dos artefactos con nombres parecidos">
	...
	`--agent-tar`. `mikroscope-agent_1.0.6_linux_arm64.tar.gz` es un archivo con el binario del agente
