...
| -------------------------------------------------------------------- | ---------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------- |
| **[Un pull del registro](#un-pull-del-registro)** — empieza por aquí | que el router llegue a Docker Hub, y que `registry-url` lo nombre      | `registry-url` ya nombra Docker Hub, o todos los contenedores del router son tuyos: una orden, nada que subir |
...
**Toma la primera salvo que algo te lo impida**, como un `registry-url` que sigue en el valor por
defecto de RouterOS en un router cuyos otros contenedores no son tuyos. Un pull del registro es una
...
`/container/add remote-image="jmrplens/mikroscope-agent:{{MIKROSCOPE_VERSION}}" …` y el plan
...
router se la descarga del que ya nombre `/container/config registry-url`. En la
RB5009 sobre la que se mide este proyecto, ese ajuste vale
`https://registry-1.docker.io`, y la orden de arriba no necesitó fijar nada antes.
RouterOS no lo trae así: desde la
[7.18](https://download.mikrotik.com/routeros/7.18/CHANGELOG) su valor por defecto
es `https://lscr.io`, el mismo que da la [documentación de contenedores](https://help.mikrotik.com/docs/spaces/ROS/pages/84901929/Container)
de MikroTik, y la imagen solo se publica en Docker Hub y en GHCR. Lee antes el
tuyo con `/container/config/print`. En un router que siga con ese valor por
defecto, la orden de arriba necesita antes
`/container/config/set registry-url=https://registry-1.docker.io`, y la
referencia de GHCR, la alternativa, necesita
`/container/config/set registry-url=https://ghcr.io`; cualquiera de los dos es
un cambio para todos los contenedores del equipo.
...
<Aside type="caution" title="El host del registro es un ajuste global del router">
	mikroscope pone solo el resto de la referencia en `remote-image=` y deja el host del registro a
	`/container/config registry-url`, que es global al equipo y compartido con cualquier otro
	contenedor que tenga. mikroscope nunca escribe ese ajuste: apuntar el registro de tu router a otro
...
Una referencia sin host —`jmrplens/mikroscope-agent:{{MIKROSCOPE_VERSION}}`— deja el registro a
