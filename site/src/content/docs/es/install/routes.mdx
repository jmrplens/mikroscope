...
| [Un pull del registro](#un-pull-del-registro)          | que el router llegue al registro; que cuadre el `registry-url` global | instalas una release y no quieres que nada aterrice en la flash        |
...
subida. La imagen se publica para `linux/amd64`, `linux/arm64` y `linux/arm/v7`,
y RouterOS toma la que necesita su arquitectura.
...
<Aside type="caution" title="El host del registro es un ajuste global del router">
	...
	su lugar y, cuando no cuadra, nombra la única orden que hay que ejecutar:
	`/container/config/set registry-url=https://ghcr.io`. Instala con `--agent-tar` si prefieres no
	tocarlo.
</Aside>
