...
that pulls from the agent, merges a RouterOS API tier at 1 Hz, and writes to eleven
...
(linux/amd64, linux/arm64), and `deploy/` has two compose stacks that pair it
with InfluxDB 3 or Prometheus and a Grafana. That image is for `forward`: the
install verbs shell out to ssh and scp, which it leaves out on purpose.
