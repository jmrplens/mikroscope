...
func apiNetPanels(b qb) []Panel {
	return []Panel{
		...
		{
			Title: "Port errors per bin — typed, from the MAC counters", Type: typeStateTL, Unit: "short", W: 24, H: 9, MinInterval: "1m",
			Mappings: []Mapping{
		...
		},
		{
			...
			Queries: b.q(
				`SELECT $__dateBin(time) AS time, concat(interface, ' ', kind) AS metric, (max(v) - min(v)) / ($__interval_ms / 1000.0) AS value FROM (SELECT time, interface, 'switched in hardware' AS kind, rx_bytes - driver_rx_byte AS v FROM mikroscope_api_ifcounters WHERE $__timeFilter(time) AND driver_rx_byte IS NOT NULL UNION ALL SELECT time, interface, 'CPU fast path', fp_rx_byte FROM mikroscope_api_ifcounters WHERE $__timeFilter(time) AND driver_rx_byte IS NOT NULL UNION ALL SELECT time, interface, 'CPU slow path', driver_rx_byte - fp_rx_byte FROM mikroscope_api_ifcounters WHERE $__timeFilter(time) AND driver_rx_byte IS NOT NULL) GROUP BY 1, 2 ORDER BY 1`,
				``,
			),
		},
		{
		...
		{
			Title: "Interface drops — rx, tx and tx-queue", Type: typeStateTL, Unit: "short", W: 24, H: 9,
			Mappings: []Mapping{
