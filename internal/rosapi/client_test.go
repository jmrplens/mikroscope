...
func TestDialTLSTimeout(t *testing.T) {
	// The test dials a broadcast address and asks for a timeout. On Linux and
	// macOS the connect hangs until the deadline and the error is a net.Error
	// whose Timeout() is true; on Windows the stack refuses it outright
	// (WSAEACCES), so the error is real but it is not a timeout, and the
	// assertion below is about the platform rather than about this client.
