...
type Writer interface {
	// BeginSentence starts a new sentence.
	BeginSentence()
	// WriteWord appends one word to the current sentence.
	WriteWord(word string)
	// EndSentence terminates and flushes the current sentence.
	EndSentence() error

	// Close releases the writer; further writes report io.EOF.
	Close()
}
...
// encodeLength packs a word length into the RouterOS wire prefix (1-5 bytes).
// #nosec G115 -- each byte() conversion keeps exactly the low bits the shift
// exposes; that truncation IS the wire format, guarded by the switch ranges.
//
