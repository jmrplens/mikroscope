...
// in a second shell appends one row while `record` is running, and the
// recorder's next row — a typed note or a gap — has to land after it, not on
// top of it. The README, the walkthrough and the CLI's usage line all say
// marking a running recording works.
//
// Neither writer holds the file open: each one opens, appends and closes. A
// held-open writer passes this on Linux and macOS, where two appending
// descriptors interleave by POSIX, and fails on Windows, where the second
// handle keeps its own offset.
func TestAMarkerFromAnotherShellSurvivesTheRecordersNextWrite(t *testing.T) {
	t.Parallel()
	prefix := filepath.Join(t.TempDir(), "cap")
	header, err := openOutput(prefix + ".markers.csv")
	if err != nil {
		t.Fatal(err)
	}
	if _, err = header.WriteString("wall_ns,wall_utc,seq,kind,label\n"); err != nil {
		t.Fatal(err)
	}
	...
	// The recorder, through the path it really uses.
	rc := &Recorder{Opts: Options{Prefix: prefix}}
	rc.mark(Marker{WallNS: 2, Seq: 2, Kind: "gap"})

	body, err := os.ReadFile(prefix + ".markers.csv") // #nosec G304 -- a path this test made
	if err != nil {
		t.Fatal(err)
	}
	want := "wall_ns,wall_utc,seq,kind,label\n" +
		"1,1970-01-01T00:00:00.000000001Z,1,note,queue tree applied\n" +
		"2,1970-01-01T00:00:00.000000002Z,2,gap,\n"
	if string(body) != want {
		t.Fatalf("the recorder wrote over the marker.\n got:\n%s\nwant:\n%s", body, want)
	}
	if rc.nMark != 1 {
		t.Errorf("the recorder counted %d markers of its own, want 1", rc.nMark)
	}
}
