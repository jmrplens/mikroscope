...
func (rc *Recorder) mark(m Marker) {
	...
	_ = rc.markers.Write(markerRow(m))
	rc.markers.Flush()
	rc.nMark++
}
...
// was there.
//
// O_APPEND is load-bearing on the markers file. `mark` in another shell opens
// that file O_APPEND and writes a row (AppendMarkers below); without O_APPEND
// here the recorder keeps its own file offset for the whole run and its next
// write lands on top of that row. The README, the walkthrough and the CLI's own
// usage line all say marking a RUNNING recording works, and until 2026-09-15 it
// silently did not. O_TRUNC still empties the file at open, so a re-run does not
...
func (rc *Recorder) open() (*outputs, func(), error) {
	...
	rc.markers = csv.NewWriter(mf)
	_ = rc.markers.Write(MarkerHeader)
	rc.markers.Flush()
