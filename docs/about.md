...
What works end to end on a MikroTik RB5009, what the agent costs there, what was found and not fixed, and what each release publishes.
...
is [1.2.2](https://github.com/jmrplens/mikroscope/releases/tag/v1.2.2).
...
[One device, two RouterOS versions](https://jmrp.io/docs/mikroscope/about/status/#one-device-two-routeros-versions).
...
forms never have.** The rules number 14
for InfluxDB 3, 15 for Prometheus
and 10 for PostgreSQL. The load of
...
traffic; [the alert rules page](https://jmrp.io/docs/mikroscope/dashboards/alerts/#the-rules) has the
...
`registry-url` (verified on RB5009UG+S+, RouterOS 7.24.4, 2026-09-24), and Docker Hub
served the agent with the device's registry username and password cleared, then
restored (verified on RB5009UG+S+, RouterOS 7.24.4, 2026-09-24). That is the form `--remote-image` now
...
The repository's [`VERSION`](https://github.com/jmrplens/mikroscope/blob/main/VERSION) file holds **1.2.2**, which the
...
- **CLI archives** for linux and freebsd on amd64, arm64 and arm (ARMv7), and
  ...
  each carrying the MIT [`LICENSE`](https://github.com/jmrplens/mikroscope/blob/main/LICENSE) and [`README.md`](https://github.com/jmrplens/mikroscope/blob/main/README.md).
...
- **Side-loadable agent image tars**, four of them —
  ...
  router.
- **The agent image in two registries**, `jmrplens/mikroscope-agent:1.2.2` on
  Docker Hub and `ghcr.io/jmrplens/mikroscope-agent:1.2.2` on GHCR (each also
...
- **The collector image**, `jmrplens/mikroscope:1.2.2` on Docker Hub and
  `ghcr.io/jmrplens/mikroscope:1.2.2` on GHCR (each also tagged `latest`), over
...
- [Lineage and licence](https://jmrp.io/docs/mikroscope/about/lineage/): where the deployment code and the API client
  came from.
- [Releases](https://jmrp.io/docs/mikroscope/about/changelog/): the current release and its date, and where every
...
The current release of mikroscope is [1.2.2](https://github.com/jmrplens/mikroscope/releases/tag/v1.2.2), released on 2026-09-24. Neither is typed
...
publishes](https://jmrp.io/docs/mikroscope/about/status/#what-the-release-publishes) on the status page.

### See also

- [Four ways to install](https://jmrp.io/docs/mikroscope/install/routes/): which of those files each install route
  takes, and how to verify it.
- [Getting the CLI onto your machine](https://jmrp.io/docs/mikroscope/install/cli/): the archive, `go install` or the
  install script, at the current release.
- [Where the project stands](https://jmrp.io/docs/mikroscope/about/status/): what the current release has run on, and
  what it has not.
...
- [Where the project stands](https://jmrp.io/docs/mikroscope/about/status/): what works today, and what the release
  publishes.
- [Releases](https://jmrp.io/docs/mikroscope/about/changelog/): the current release, and where the history of every
