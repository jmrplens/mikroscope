...
What `mikroscope forward` does between the agent and your stores — pull, merge, derive, fan out — and what it promises when a store is slow.
...
The values the collector computes beside the raw samples — memory pressure, per-packet PMU cost, packets per interrupt, the burst flag and the fast-path share — and when each one is withheld.
...
What the agent establishes about the board with no RouterOS API — identity, ceilings, cadences — and how the collector hands it to every sink as facts rather than samples.
