...
Eight readings of kernel and port data from one RB5009 — two production faults found without looking for them, three events provoked on purpose, two that needed no provoking, and the idle shape they are read against.
...
A layer-2 loop on the reference router’s production network that RouterOS’s log and port monitor never showed, found by the agent’s kernel-log source, diagnosed from a rate, and gone the same day after a firmware update, confirmed two independent ways. It came back a week later.
