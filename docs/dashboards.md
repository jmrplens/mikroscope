...
## Two dashboards, one panel list

What the InfluxDB 3 and Prometheus dashboards contain, section by section, in the order they are read, and why the two stores do not carry the same panels.
...
Both dashboards are in Grafana's shareable export format: the datasource is a `${DS_MIKROSCOPE}`
...
| Section (Grafana row) | InfluxDB | Prometheus |
| --- | --- | --- |
| Overview | 12 | 12 |
| CPU and scheduler | 11 | 9 |
| Memory and load | 9 | 9 |
| Connections | 9 | 3 |
| Interface traffic | 13 | 11 |
| Detections and captures | 5 | 4 |
| Network receive path | 9 | 7 |
| Forwarding cost (derived) | 4 | 4 |
| Interrupts and softirqs | 12 | 10 |
| Temperature and clock | 10 | 2 |
| Kernel log | 7 | 6 |
| CPU: how long a core stayed busy | 3 | 4 |
| Memory: fragmentation | 3 | 1 |
| Memory: reclaim and page faults | 8 | 6 |
| Memory: detail and cross-checks | 5 | 3 |
| Hardware counters (PMU) | 11 | 7 |
| Flash wear | 7 | no row |
| NAND health (ECC) | 2 | 2 |
| RouterOS API cross-checks — CPU and memory | 9 | 9 |
| The observer | 11 | 9 |
| The observer: sampler timing and self events | 3 | 7 |
| This device | 3 | 3 |
| Not available on this device | 5 | 5 |
| **Total** | **171** | **133** |
...
- [Two dashboards, one panel list](https://jmrp.io/docs/mikroscope/dashboards/): every section and panel, and which store
  carries each.
...
| --- | --- | --- | --- | --- | --- | --- |
| `mikroscope-agent-silent` | fewer than 1 new sample reached the store in the last 2 minutes | < 1 | critical | 2m | Alerting | both |
| `mikroscope-softnet-drops` | `softnet_stat` dropped a packet in the last 5 minutes | > 0 | critical | 0s | OK | both |
| `mikroscope-oom-kill` | `/proc/vmstat` `oom_kill` moved in the last 5 minutes | > 0 | critical | 0s | OK | both |
| `mikroscope-detections` | any detection in the last 5 minutes | > 0 | warning | 0s | OK | both |
| `mikroscope-thermal-near-critical` | a zone at or above 85 % of its own critical trip | > 0 | critical | 1m | OK | both |
| `mikroscope-conntrack-near-limit` | `nf_conntrack` active objects above 0.8 of the kernel's limit | > 0.8 | warning | 5m | OK | both |
| `mikroscope-ticks-slipped` | the sampler slipped a tick in the last 5 minutes | > 0 | warning | 5m | OK | Prometheus only |
| `mikroscope-agent-oom` | the agent's own cgroup recorded an OOM kill in the last 5 minutes | > 0 | critical | 0s | OK | both |
...
| `mikroscope-ecc-failure` | the NAND reported an uncorrectable ECC failure in the last hour | > 0 | critical | 0s | OK | both |
...
- [Two dashboards, one panel list](https://jmrp.io/docs/mikroscope/dashboards/): the panels whose fault counters these
