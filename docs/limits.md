...
What the agent costs a MikroTik RB5009 at 10 to 100 Hz, measured from its own cgroup, and how to measure it on a device that is not this one.
...
MikroTik RB5009 **2.69 % of one core and 13.2 MiB RSS**,
...
a number: [the rate ceiling](https://jmrp.io/docs/mikroscope/cost/rate-ceiling/) is that table.
...
Six measured runs at 10, 20, 50 and 100 Hz on an RB5009 at its shipped configuration, none of which lost a sample, and what sampling faster buys.
...
[provoked on purpose](https://jmrp.io/docs/mikroscope/playbooks/). They were not taken on one day:
...
Why a MikroTik router’s CPU reading is no finer than the kernel’s 10 ms tick, the one source that reads beneath it, and how far back the agent remembers.
...
A tick lasts 10 ms. [`/proc/stat`](https://man7.org/linux/man-pages/man5/proc_stat.5.html) does not count time, it counts
...
Which files in a RouterOS container are the router’s and which its own, why privileged does not change that, and the conntrack count that gets through.
...
Why the agent’s RouterOS container runs privileged=yes by default, what that makes readable, and the network, processes and sensors it still cannot reach.
...
Which kernel sources the agent reads every tick and which less often, the named reason each reports, and FLOOR_HZ, which turns every floor off.
...
The floors are named constants in [`internal/agent/source.go`](https://github.com/jmrplens/mikroscope/blob/main/internal/agent/source.go) rather than numbers buried
in the logic.
