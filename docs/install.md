...
```sh
curl -fsSL .../install.sh | VERSION=1.2.2 BIN_DIR=~/bin bash
./install.sh --version 1.2.2 --dir ~/bin      # the same, with the file downloaded first
```
...
- **Windows**
  ...
  2. Check it in PowerShell, against the line for your file in `checksums.txt`:
     ...
     ```powershell
     Get-FileHash .\mikroscope_1.2.2_windows_x86_64.zip -Algorithm SHA256
     ```

  3. Unpack it somewhere permanent and put that folder on your `PATH`:

     ```powershell
     Expand-Archive .\mikroscope_1.2.2_windows_x86_64.zip -DestinationPath $HOME\mikroscope
...
mikroscope install --router user@192.168.88.1 \
  --remote-image jmrplens/mikroscope-agent:1.2.2
```

Nothing is uploaded, no tar lands on the device, and `uninstall` has no file to
account for: the container step becomes
`/container/add remote-image="registry-1.docker.io/jmrplens/mikroscope-agent:1.2.2" …`
and the plan prints
`the router pulls registry-1.docker.io/jmrplens/mikroscope-agent:1.2.2 (nothing is uploaded)`
...
`ghcr.io/jmrplens/mikroscope-agent:1.2.2` on GHCR. Both carry `linux/amd64`,
...
`registry-1.docker.io/jmrplens/mikroscope-agent:1.2.2`, the host Docker Hub's
registry answers on; any other host is kept as given, so the GHCR reference goes
out as `ghcr.io/jmrplens/mikroscope-agent:1.2.2`. RouterOS takes a registry host
...
reference, `--remote-image <mirror-host>/jmrplens/mikroscope-agent:1.2.2`,
...
1. Download `mikroscope_1.2.2_<os>_<arch>.tar.gz` (`.zip` on Windows) and the
...
3. Unpack the CLI and install:
   ...
   ```sh
   tar xzf mikroscope_1.2.2_linux_x86_64.tar.gz
   ./mikroscope install --router user@192.168.88.1 \
...
> `mikroscope-agent_1.2.2_linux_arm64.tar.gz` is an archive of the bare agent binary, for reading or
